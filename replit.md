...
Gestionale per un magazzino solidale: tracciamento prodotti/lotti (FEFO), CRM beneficiari, consegne, trasferimenti, volontari, report — più modulo **Unità di Strada (UDS)** e scoping per **Area Operativa / Zona**.
...
- **Ruoli Volontari** (pagina admin sotto amministrazione, tabella `ruoli_volontari`): picklist configurabile dei ruoli che sostituisce le 4 vecchie opzioni hardcoded.
  ...
  - Display via `t(\`volontari.roles.<nome>\`, {defaultValue: nome})`. Select mostra i ruoli attivi O il valore corrente della riga. GET leggibile da logistica; mutazioni requireAdmin; il delete ritira solo l'opzione. Ruoli GLOBALI (no scoping area operativa/centro).
...
- **Flag `uds` & invariante area operativa**: `beneficiari.uds` (boolean, default false) guida l'appartenenza UDS, indipendente da `zonaUdsId`. Una persona `uds=true` deve SEMPRE avere area operativa non-null (altrimenti trapela tra tutte le area operativa).
  - Imposto server-side su `POST` E `PATCH /beneficiari` per ogni percorso che lo setta (form UDS, toggle "anche UDS" standard, toggle dettaglio): il flag è normalizzato via `toBool` (così `uds:"true"`/`1` non bypassa per type-confusion), poi i chiamanti scoped pinnano la propria area operativa (incl. righe legacy null su PATCH) e i chiamanti globali devono fornire una area operativa o ricevono 400. Il form beneficiari standard richiede la area operativa lato client quando "anche UDS" è attivo per un admin globale.
- **UDS Anagrafica** (`pages/uds-anagrafica.tsx`): lista persone filtrate `uds=true`, scoping area operativa (hard, server-side) + filtro **Zona** che default sulla zona dell'operatore (`user.zonaUdsId`), "Tutta la area operativa" = tutte. Il super-admin globale (`user.areaOperativaId==null`) ha anche un filtro **Area Operativa**.
  - Form di creazione con campi beneficiari COMPLETI incl. select **Centro di ascolto** (`useListCentriAscolto`, "Nessuno"=null) E toggle **anche UDS** (`uds`, default on) che GATE i campi area operativa/zona (nascosti se off); area operativa richiesta lato client (zod `superRefine`) solo se `isGlobal && uds`; `zonaUdsId` rimosso dal payload quando `uds` off.
...
- **UDS Interventi** (`pages/uds-interventi.tsx`): picker persona scoping area operativa + filtro **Zona** (default `user.zonaUdsId`, "Tutte le zone"=tutte) e, per admin globali, filtro **Area Operativa**; query `useListBeneficiari({uds:true, areaOperativaId?, zonaUdsId?})`. Cambiare un filtro resetta la persona selezionata.
...
- **Anti-doppione fuzzy** (`GET /beneficiari/cerca-simili`, prima di `/beneficiari/:id`): similarità Postgres `pg_trgm` su nome+cognome (e invertito via `GREATEST`), soprannome (*0.5), telefono (esatto 0.5 / sim *0.3), dataNascita (esatto +0.4); soglia 0.2, `ORDER BY score`, LIMIT 10; supporta `excludeId`.
  - **Area Operativa-HARD-scoped** via `callerAreaOperativaId` (scoped → propria area operativa O NULL legacy; globale può restringere con `?areaOperativaId`). Estensione abilitata idempotentemente allo startup (`lib/dbInit.ts`).
...
- Ogni `mense` appartiene a una `area operativa` e collega in modo univoco un `magazzini` attivo già configurato esplicitamente come tipo `mensa`; la creazione non converte magazzini logistici esistenti. Prodotti, lotti, scadenze, FEFO, movimenti, trasferimenti e PDF restano quelli logistici esistenti.
- `tessere_beneficiari` è trasversale ai servizi e conserva solo un token opaco. Il backfill di `beneficiari.codice` è esclusivamente compatibilità per tessere legacy già stampate; ogni nuova emissione usa un token casuale `MS-...` privo di dati personali. Gli accessi risolvono l'identità lato server e restituiscono alla postazione esclusivamente identità operativa, Mensa assegnata e informazioni alimentari necessarie.
- `mensa_abilitazioni` conserva lo storico. Le principali `attiva` con `data_fine` precedente alla data di servizio vengono riconciliate in `scaduta`, con audit, prima di crearne una nuova. Un operatore territoriale opera nello scope territoriale corrente, oggi implementato tramite `areaOperativaId`: una Mensa diversa è autorizzabile con motivazione solo nello stesso territorio. La policy è centralizzata in `canUseMensaException` per poter adottare in futuro un eventuale `areaId` senza distribuire il confronto nelle route. Un altro territorio è sempre negato e i dati personali vengono oscurati.
...
### Scoping per Area Operativa / Zona UDS
- Asse top-level (additivo a centro + aree). **Area Operativa** = confine di visibilità HARD (un operatore non vede mai persone di un'altra area operativa). **Zona UDS** (municipio) sotto una area operativa = preferenza SOFT (vede prima la propria zona, può filtrare tutta la area operativa).
- Colonne: `utenti.areaOperativaId` (nullable = super-admin globale) + `utenti.zonaUdsId` (nullable = tutte le zone); `centri_ascolto.areaOperativaId`, `magazzini.areaOperativaId`, `beneficiari.areaOperativaId` + `beneficiari.zonaUdsId` + `beneficiari.soprannome`. Tabelle `area operativa` + `zone_uds` (FK `areaOperativaId`).
- `/auth/me` espone `areaOperativaId/areaOperativaNome/zonaUdsId/zonaUdsNome`. Pagine CRUD admin **Area Operativa** + **Zone UDS** (area amministrazione). Il form Utenti ha un select Area Operativa + un select Zona UDS dipendente ("Tutte le zone" = null; zona disabilitata finché non si sceglie una area operativa).
- I centri di ascolto hanno un select **Area Operativa** nel form admin (solo admin globali; gli scoped ereditano la propria area operativa) — necessario perché il filtro Area Operativa in Pianificazione Consegne mostra i centri della area operativa scelta.
- Gli helper di scoping vivono accanto a quelli centro (filtro area operativa hard su col diretta / via beneficiario / via magazzini visibili); guardie IDOR su PATCH rispecchiano il pattern centro. `prodotti` resta globale.
...
- **Cookie auth** adattivo all'ambiente (`app.ts`): su Replit (rileva `REPLIT_DOMAINS`/`REPLIT_DEV_DOMAIN`) usa `SameSite=None;Secure` + `trust proxy 1` per l'iframe di preview cross-site; in **self-host su HTTP** ripiega su `SameSite=Lax` non-Secure (un cookie `Secure` su HTTP viene scartato dal browser → ogni richiesta sembra "Non autenticato"). Override con `COOKIE_SECURE=true|false`; origini CSRF extra per self-host via `APP_ORIGINS` (origini complete separate da virgola). Per via di `SameSite=None`, una guardia CSRF Origin/Referer (da `REPLIT_DOMAINS`/`REPLIT_DEV_DOMAIN` + `APP_ORIGINS`) gira su tutte le richieste `/api` non-GET/HEAD — i POST curl devono inviare `-H "Origin: https://$REPLIT_DEV_DOMAIN"` (e `-H "X-Forwarded-Proto: https"` per ottenere il cookie Secure) o ricevono 403/nessun cookie.
