...
- **Operatore su bolle**: every bolla, trasferimento and scarico records the operator who last touched it via a nullable `operatoreId` FK → `utenti`. Stamped server-side with `req.user!.id` on create AND on every mutating action (bolle: POST/PATCH/righe add+delete/conferma/consegna/annulla; trasferimenti: POST/PATCH/avvia/conferma; scarichi: POST). All list + dettaglio endpoints join `utenti` and return `operatoreId` + a non-personal `operatoreCodice` = `matricola ?? username` (single-row joins use a leftJoin; list endpoints batch-fetch a `utenti` id→codice map). All three PDFs (bolla/trasferimento/scarico) print an "Operatore: <operatoreCodice>" line when present (bolla: under Trasportatore in the right column; trasferimento/scarico: under Note before Firme).
