...
- **UDS Anagrafica** (`pages/uds-anagrafica.tsx`): person list filtered by `uds=true`, scoped by città (hard, server-side) + a **Zona** filter defaulting to the operator's own zona (`user.zonaUdsId`), "Tutta la città" = all zones. Global super-admin (`user.cittaId==null`) also gets a **Città** filter. Create person with full beneficiari fields + auto `uds:true` (`centroAscoltoId:null`); città is REQUIRED for global admin. **Canale badge** (driven by `uds` + `centroAscoltoId`): uds+centro→Entrambi, centro→Centro, uds→UDS, neither→Non classificato.
