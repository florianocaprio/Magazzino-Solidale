...
### Unità di Strada (UDS)
- Lightweight street-outreach module (area `uds`, role "Operatore UDS" = aree `["uds"]`), NO warehouse. Reuses the shared person record (beneficiari) + interventi — a UDS person is just a beneficiario with `zonaUdsId`/`cittaId` set and `centroAscoltoId=null`. Backend `AREA_BY_SEGMENT` maps beneficiari/interventi to BOTH `["sociale","uds"]` so a UDS operator reaches those endpoints.
- **UDS Anagrafica** (`pages/uds-anagrafica.tsx`): person list scoped by città (hard, server-side) + a **Zona** filter defaulting to the operator's own zona (`user.zonaUdsId`), "Tutta la città" = all zones. Global super-admin (`user.cittaId==null`) also gets a **Città** filter. Create person as UDS (`centroAscoltoId:null`); città is REQUIRED for global admin (else the hard boundary breaks). **Canale badge**: zona+centro→Entrambi, centro→Centro, zona→UDS, neither→Non classificato.
- **UDS Interventi** (`pages/uds-interventi.tsx`): pick a person (città-scoped) → list their interventi; create street intervention = `dataIntervento`/`tipoIntervento` (ascolto/distribuzione/orientamento/salute/altro) + bisogni as free note in `descrizione` + materiale described in `note`. Per-person XLSX/PDF export via `ExportButtons`. No new backend — reuses `GET/POST /interventi`.
