...
- **Multilingua menù (i18n)**: the sidebar navigation (group labels + item titles) and footer (logout/no-role labels) are translatable across 6 languages — Italiano, Español, English, Français, Deutsch, العربية (Arabic). A language Select lives in the sidebar footer (always accessible to every user, above the user/logout row). Powered by `i18next` + `react-i18next`; all strings + setup in `src/lib/i18n.ts` (single `translation` namespace with `nav.groups.*`, `nav.items.*`, `common.*`). NAV_ITEMS carry stable `key`/`groupKey` mapped to translation keys (not literal Italian strings). Choice persists to `localStorage` (`ms-lang`, default `it`). Arabic sets `<html dir="rtl">` via `applyDirection()` on init + `languageChanged`. Only the menu chrome is translated for now (page bodies/forms remain Italian); infra is extensible — add namespaces/keys and `useTranslation()` in pages to grow coverage.
