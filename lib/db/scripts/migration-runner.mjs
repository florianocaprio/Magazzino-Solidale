...
export const LEGACY_LOCK_KEY = "magazzino-solidale:db-updates";
...
export async function loadMigrationPlan({ updatesDirectory, manifestPath }) {
  ...
  const legacyBoundaryFilename = manifest.files.at(-1)?.filename ?? null;
  const legacyBoundaryViolations = legacyBoundaryFilename
    ? files.filter(
        (file) =>
          !manifestNames.has(file.filename) &&
          file.filename <= legacyBoundaryFilename,
      )
    : [];
  if (legacyBoundaryViolations.length) {
    const filename = legacyBoundaryViolations[0].filename;
    throw new MigrationRunnerError(
      "MIGRATION_LEGACY_BOUNDARY_VIOLATION",
      `Migration non presente nel manifest entro il confine legacy: ${filename} <= ${legacyBoundaryFilename}`,
      { filename, legacyBoundaryFilename },
    );
  }
  ...
  return { files, manifest, legacyBoundaryFilename };
}
...
const MIGRATION_LOCKS = [
  {
    name: "legacy",
    key: LEGACY_LOCK_KEY,
    trySql: "SELECT pg_try_advisory_lock(hashtext($1)) AS acquired",
    releaseSql: "SELECT pg_advisory_unlock(hashtext($1)) AS released",
  },
  {
    name: "global",
    key: GLOBAL_LOCK_KEY,
    trySql: "SELECT pg_try_advisory_lock(hashtextextended($1, 0)) AS acquired",
    releaseSql:
      "SELECT pg_advisory_unlock(hashtextextended($1, 0)) AS released",
  },
];

async function acquireMigrationLocks(
  client,
  ...
  for (const lock of MIGRATION_LOCKS) {
    while (true) {
      const result = await client.query(lock.trySql, [lock.key]);
      if (result.rows[0].acquired === true) {
        acquiredLocks.push(lock);
        break;
      }
      const remainingMs = lockTimeoutMs - (Date.now() - startedAt);
      if (remainingMs <= 0) {
        throw new MigrationRunnerError(
          "MIGRATION_LOCK_TIMEOUT",
          `Timeout acquisizione lock migration ${lock.name} dopo budget complessivo di ${lockTimeoutMs} ms`,
          { lockName: lock.name, lockKey: lock.key, lockTimeoutMs },
        );
      }
      await wait(Math.min(lockPollIntervalMs, remainingMs));
    }
  }
}

async function releaseMigrationLocks(client, acquiredLocks, logger) {
  for (const lock of acquiredLocks.reverse()) {
    try {
      const result = await client.query(lock.releaseSql, [lock.key]);
      if (result.rows[0].released !== true) {
        log(
          logger,
          "error",
          `MIGRATION_LOCK_RELEASE_FAILED: lock=${lock.name} non detenuto dalla sessione`,
        );
      }
    } catch (error) {
      log(
        logger,
        "error",
        `MIGRATION_LOCK_RELEASE_FAILED: lock=${lock.name} ${sanitizeErrorMessage(error)}`,
      );
    }
  }
}
...
export async function runMigrations(options) {
  ...
  try {
    ...
    try {
      await acquireMigrationLocks(client, resolved, acquiredLocks);
    } catch (error) {
    ...
    await releaseMigrationLocks(client, acquiredLocks, resolved.logger);
    await client.end();
  }
}
