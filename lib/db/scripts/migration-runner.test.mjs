...
describe("manifest legacy", { concurrency: false }, () => {
  ...
  test("mantiene 21 file legacy e accetta Beneficiari 2.0, Reporting 2.0, Volontari 2.0 e Maps con hardening in coda", async () => {
    const plan = await loadMigrationPlan({
      updatesDirectory: realUpdatesDirectory,
      manifestPath: realManifestPath,
    });
    assert.equal(plan.manifest.files.length, 21);
    assert.equal(plan.files.length, 33);
    assert.ok(
      plan.files.some(
    ...
    assert.equal(
      plan.files.at(-8).filename,
      "20260828_reporting_2_0_final_alignment.sql",
    );
    assert.equal(
      plan.files.at(-7).filename,
      "20260829_reporting_2_0_hardening.sql",
    );
    assert.equal(
      plan.files.at(-6).filename,
      "20260830_reporting_2_0_residual_workflow_hardening.sql",
    );
    assert.equal(plan.files.at(-5).filename, "20260831_volontari_2_0.sql");
    assert.equal(
      plan.files.at(-4).filename,
      "20260901_volontari_2_0_review_hardening.sql",
    );
    assert.equal(
      plan.files.at(-3).filename,
      "20260902_volontari_2_0_scope_temporal_hardening.sql",
    );
    assert.equal(plan.files.at(-2).filename, "20260903_maps_geocode_cache.sql");
    assert.equal(
