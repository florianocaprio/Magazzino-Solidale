...
describe("manifest legacy", { concurrency: false }, () => {
  ...
  test("mantiene 21 file legacy e accetta Beneficiari 2.0, Reporting 2.0 e Volontari 2.0 con hardening in coda", async () => {
    const plan = await loadMigrationPlan({
      updatesDirectory: realUpdatesDirectory,
      manifestPath: realManifestPath,
    });
    assert.equal(plan.manifest.files.length, 21);
    assert.equal(plan.files.length, 31);
    assert.ok(
      plan.files.some(
    ...
    assert.equal(plan.files.at(-3).filename, "20260831_volontari_2_0.sql");
    assert.equal(
