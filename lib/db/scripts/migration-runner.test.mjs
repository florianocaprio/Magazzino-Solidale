...
describe("manifest legacy", { concurrency: false }, () => {
  ...
  test("mantiene 21 file legacy e accetta Beneficiari 2.0 e le migration 2.0C/R1/R2 in coda", async () => {
    const plan = await loadMigrationPlan({
    ...
    assert.ok(plan.files.some((file) => file.filename === "20260827_beneficiari_2_0_fse.sql"));
    assert.equal(plan.files.at(-1).filename, "20260827_beneficiari_2_0_fse.sql");
    assert.equal(
