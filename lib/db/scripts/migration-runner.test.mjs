  ...
  test("mantiene 21 file legacy e accetta M2/M3/M4/M5A in coda", async () => {
    ...
    assert.ok(
      plan.files.some(
        (file) =>
          file.filename ===
          "20260830_reporting_2_0_residual_workflow_hardening.sql",
      ),
    );
    assert.deepEqual(
      plan.files.slice(-18).map((file) => file.filename),
      [
        "20260828_reporting_2_0_final_alignment.sql",
        "20260829_reporting_2_0_hardening.sql",
        "20260830_reporting_2_0_residual_workflow_hardening.sql",
        "20260831_volontari_2_0.sql",
        "20260901_volontari_2_0_review_hardening.sql",
        "20260902_volontari_2_0_scope_temporal_hardening.sql",
        "20260903_maps_geocode_cache.sql",
        "20260904_volontari_2_0_manual_test_blockers.sql",
        "20260911_m1c_audit_eventi.sql",
        "20260916_m1c_audit_hardening.sql",
        "20260917_m2_catalogo_lotti_logici.sql",
        "20260918_m3a_carico_pratiche.sql",
        "20260918_z_m3b_fse_import_practices.sql",
        "20260918_za_m3b_fse_identity_aliases.sql",
        "20260919_m4a_documenti_destinatari.sql",
        "20260923_m4b1_prenotazioni_trasferimenti.sql",
        "20260924_m4b2_rientri_trasporto.sql",
        "20260925_m5a_richieste_magazzino.sql",
      ],
    );
    assert.equal(
...
const databaseTestName =
  process.env.M5A_RUNNER_TEST_DATABASE_NAME ??
  `migration_ledger_${process.pid}_${Date.now()}`;
const m5aExpectedSystemId = process.env.M5A_RUNNER_TEST_SYSTEM_IDENTIFIER;
const m5aSessionToken = process.env.M5A_RUNNER_TEST_SESSION_TOKEN;
const m5aExpectedPort = process.env.M5A_RUNNER_TEST_PORT;
let databaseUrl;
let adminClient;
  ...
  try {
    if (m5aSessionToken) {
      ...
      assert.deepEqual(identity, {
        ...
        system_identifier: m5aExpectedSystemId,
        marker: `m5a-test:${m5aSessionToken}`,
      });
    }
...
describe(
  ...
  () => {
    before(async () => {
      ...
      if (m5aSessionToken) {
        assert.match(databaseTestName, /^m5a_runner_[0-9a-f]{12}$/);
        assert.equal(adminUrl.hostname, "127.0.0.1");
        assert.equal(adminUrl.port, m5aExpectedPort);
      ...
      if (m5aSessionToken) {
        const identity = (
          await adminClient.query(
            "SELECT (pg_control_system()).system_identifier::text AS system_identifier, (SELECT token FROM m5a_test_meta.session_marker) AS token",
          )
        ).rows[0];
        assert.equal(identity.system_identifier, m5aExpectedSystemId);
        assert.equal(identity.token, m5aSessionToken);
      }
      await adminClient.query(
      ...
      if (m5aSessionToken) {
        await adminClient.query(
          `COMMENT ON DATABASE ${quoteIdentifier(databaseTestName)} IS 'm5a-test:${m5aSessionToken}'`,
        );
      }
...
after(async () => {
  ...
  if (adminClient && databaseUrl) {
    if (m5aSessionToken) {
      const identity = (
        await adminClient.query(
          "SELECT (pg_control_system()).system_identifier::text AS system_identifier, (SELECT shobj_description(oid, 'pg_database') FROM pg_database WHERE datname = $1) AS marker",
          [databaseTestName],
        )
      ).rows[0];
      assert.equal(identity.system_identifier, m5aExpectedSystemId);
      assert.equal(identity.marker, `m5a-test:${m5aSessionToken}`);
    }
    await adminClient.query(
