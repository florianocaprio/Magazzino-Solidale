...
describe("manifest legacy", { concurrency: false }, () => {
  ...
  test("mantiene 21 file legacy e accetta gli hardening fino all'audit M1C in coda", async () => {
    const plan = await loadMigrationPlan({
      updatesDirectory: realUpdatesDirectory,
      manifestPath: realManifestPath,
    });
    assert.equal(plan.manifest.files.length, 21);
    assert.equal(plan.files.length, 34);
    assert.ok(
      plan.files.some(
    ...
    assert.deepEqual(
      plan.files.slice(-9).map((file) => file.filename),
      [
