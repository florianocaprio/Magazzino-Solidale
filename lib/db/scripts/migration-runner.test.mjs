...
describe("manifest legacy", { concurrency: false }, () => {
  ...
  test("mantiene 21 file legacy e accetta Beneficiari 2.0, Reporting 2.0 e Volontari 2.0 in coda", async () => {
    const plan = await loadMigrationPlan({
    ...
    assert.equal(plan.files.at(-2).filename, "20260831_volontari_2_0.sql");
    assert.equal(
