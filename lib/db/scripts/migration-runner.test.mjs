...
describe("manifest legacy", { concurrency: false }, () => {
  ...
  test("mantiene 21 file legacy e accetta le migration 2.0C/R1 in coda", async () => {
    const plan = await loadMigrationPlan({
      updatesDirectory: realUpdatesDirectory,
      manifestPath: realManifestPath,
    });
    assert.equal(plan.manifest.files.length, 21);
    assert.equal(plan.files.length, 23);
    assert.equal(
      plan.files.at(-1).filename,
      "20260825_magazzino_2_0c_r1_reporting_hardening.sql",
    );
    assert.equal(
