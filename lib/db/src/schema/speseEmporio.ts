...
export const speseEmporioTable = pgTable("spese_emporio", {
  ...
  sessioneCassaId: integer("sessione_cassa_id")
    .notNull()
    .references(() => sessioniCassaEmporioTable.id)
    .unique(),
  accessoEmporioId: integer("accesso_emporio_id")
    .notNull()
    .references(() => consegneTable.id),
  ...
  totaleCreditoConsumati: decimal("totale_credito_consumati", {
    precision: 10,
    scale: 2,
  }).notNull(),
  saldoPrima: decimal("saldo_prima", { precision: 10, scale: 2 }).notNull(),
  saldoDopo: decimal("saldo_dopo", { precision: 10, scale: 2 }).notNull(),
  statoSpesa: varchar("stato_spesa", { length: 40 })
    .notNull()
    .default("chiusa"),
  operatoreChiusuraId: integer("operatore_chiusura_id").references(
    () => utentiTable.id,
  ),
  emailBollaStato: varchar("email_bolla_stato", { length: 40 })
    .notNull()
    .default("non_preparata"),
  ...
  emailBollaOperatoreId: integer("email_bolla_operatore_id").references(
    () => utentiTable.id,
  ),
...
export const speseEmporioRigheTable = pgTable("spese_emporio_righe", {
  ...
  sessioneCassaRigaId: integer("sessione_cassa_riga_id").references(
    () => sessioniCassaEmporioRigheTable.id,
  ),
  ...
  creditoTotale: decimal("credito_totale", {
    precision: 10,
    scale: 2,
  }).notNull(),
  scaricoId: integer("scarico_id").references(() => scarichiTable.id),
