...
export const fseImportBatchesTable = pgTable(
  "fse_import_batches",
  {
  ...
  (table) => [
    index("fse_import_batches_scope_idx").on(
      table.centroAscoltoId,
      table.areaOperativaId,
    ),
    check(
      "fse_import_batches_stato_check",
    ...
    check(
      ...
      sql`${table.hashContenutoNormalizzato} ~ '^[0-9a-f]{64}$'`,
    ),
    check(
      "fse_import_batches_counts_check",
      sql`${table.numeroRighe} >= 0 and ${table.creati} >= 0 and ${table.collegati} >= 0 and ${table.aggiornati} >= 0 and ${table.invariati} >= 0 and ${table.conflitti} >= 0 and ${table.errori} >= 0`,
    ),
  ],
);

export const fseFascicoliSocialiTable = pgTable(
  "fse_fascicoli_sociali",
  {
    id: serial("id").primaryKey(),
    beneficiarioId: integer("beneficiario_id")
      .notNull()
      .references(() => beneficiariTable.id, { onDelete: "cascade" }),
    codiceFascicolo: varchar("codice_fascicolo", { length: 255 }),
    codiceFascicoloNormalizzato: varchar("codice_fascicolo_normalizzato", {
      length: 255,
    }),
    origineFascicolo: varchar("origine_fascicolo", { length: 20 })
      .notNull()
      .default("interno"),
    ...
    tipologiaAttivitaImportata: varchar("tipologia_attivita_importata", {
      length: 80,
    }),
    statoAttualeImportato: varchar("stato_attuale_importato", { length: 80 }),
    dataRiferimento: date("data_riferimento"),
    origineDato: varchar("origine_dato", { length: 30 }),
    attendibilitaDato: varchar("attendibilita_dato", { length: 30 }),
    versione: integer("versione").notNull().default(1),
    ultimoImportBatchId: integer("ultimo_import_batch_id").references(
      () => fseImportBatchesTable.id,
      { onDelete: "set null" },
    ),
    ultimoImportAt: timestamp("ultimo_import_at", { withTimezone: true }),
    ultimoExportAt: timestamp("ultimo_export_at", { withTimezone: true }),
    hashUltimaRigaImportata: varchar("hash_ultima_riga_importata", {
      length: 64,
    }),
    dataCreazione: timestamp("data_creazione", { withTimezone: true })
      .notNull()
  ...
  (table) => [
    uniqueIndex("fse_fascicoli_sociali_beneficiario_uidx").on(
      table.beneficiarioId,
    ),
    uniqueIndex("fse_fascicoli_sociali_codice_norm_uidx")
      .on(table.codiceFascicoloNormalizzato)
      .where(sql`${table.codiceFascicoloNormalizzato} is not null`),
    check(
      "fse_fascicoli_sociali_origine_fascicolo_check",
      sql`${table.origineFascicolo} in ('interno','import_fse')`,
    ),
    check(
    ...
    check(
      ...
      sql`(${table.origineStranieraMinoranze} is null or ${table.origineStranieraMinoranze} >= 0) and (${table.cittadiniPaesiTerzi} is null or ${table.cittadiniPaesiTerzi} >= 0) and (${table.senzaTettoEsclusioneAbitativa} is null or ${table.senzaTettoEsclusioneAbitativa} >= 0) and (${table.personeDisabilita} is null or ${table.personeDisabilita} >= 0)`,
    ),
    check("fse_fascicoli_sociali_versione_check", sql`${table.versione} >= 1`),
  ],
);

export const fseFascicoliSocialiSnapshotTable = pgTable(
  "fse_fascicoli_sociali_snapshot",
  {
    id: serial("id").primaryKey(),
    beneficiarioId: integer("beneficiario_id")
      .notNull()
    ...
    origineSnapshot: varchar("origine_snapshot", { length: 30 }).notNull(),
    ...
    versioneProfilo: integer("versione_profilo").notNull(),
    codiceFascicolo: varchar("codice_fascicolo", { length: 255 }),
    numeroComponenti: integer("numero_componenti"),
    donne: integer("donne"),
    uomini: integer("uomini"),
    eta017: integer("eta_0_17"),
    eta1829: integer("eta_18_29"),
    eta3064: integer("eta_30_64"),
    eta65Plus: integer("eta_65_plus"),
    origineStranieraMinoranze: integer("origine_straniera_minoranze"),
    personeDisabilita: integer("persone_disabilita"),
    cittadiniPaesiTerzi: integer("cittadini_paesi_terzi"),
    senzaTettoEsclusioneAbitativa: integer("senza_tetto_esclusione_abitativa"),
    tipologiaAttivita: varchar("tipologia_attivita", { length: 80 }),
    statoAttuale: varchar("stato_attuale", { length: 80 }),
    attendibilitaDato: varchar("attendibilita_dato", { length: 30 }),
    hashCanonico: varchar("hash_canonico", { length: 64 }).notNull(),
    metadatiQualita: jsonb("metadati_qualita")
      .$type<Record<string, unknown>>()
      .notNull()
      .default({}),
    riferimentoSorgente: varchar("riferimento_sorgente", { length: 255 }),
    dataCreazione: timestamp("data_creazione", { withTimezone: true })
      .notNull()
      .defaultNow(),
  },
  (table) => [
    index("fse_fascicoli_snapshot_asof_idx").on(
      table.beneficiarioId,
      table.dataRiferimento,
      table.versioneProfilo,
    ),
    index("fse_fascicoli_snapshot_data_idx").on(table.dataRiferimento),
    uniqueIndex("fse_fascicoli_snapshot_hash_uidx").on(
      table.beneficiarioId,
      table.dataRiferimento,
      table.hashCanonico,
    ),
    ...
    check(
      "fse_fascicoli_snapshot_version_check",
      sql`${table.versioneProfilo} >= 1`,
    ),
    check(
      "fse_fascicoli_snapshot_hash_check",
      sql`${table.hashCanonico} ~ '^[0-9a-f]{64}$'`,
    ),
    check(
...
export type FseFascicoloSocialeSnapshot =
  typeof fseFascicoliSocialiSnapshotTable.$inferSelect;
