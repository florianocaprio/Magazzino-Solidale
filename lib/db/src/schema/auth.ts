...
export const utentiTable = pgTable(
  "utenti",
  {
    ...
    emailDaAggiornare: boolean("email_da_aggiornare").notNull().default(true),
...
export const passwordResetTokensTable = pgTable(
  "password_reset_tokens",
  {
    ...
    utenteId: integer("utente_id")
      .notNull()
      .references(() => utentiTable.id, { onDelete: "cascade" }),
    tokenHash: varchar("token_hash", { length: 128 }).notNull(),
    expiresAt: timestamp("expires_at").notNull(),
    usedAt: timestamp("used_at"),
    invalidatedAt: timestamp("invalidated_at"),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    createdIp: varchar("created_ip", { length: 80 }),
    createdUserAgent: varchar("created_user_agent", { length: 500 }),
  },
  (table) => [
    uniqueIndex("password_reset_tokens_hash_unique").on(table.tokenHash),
    index("password_reset_tokens_utente_idx").on(table.utenteId),
