...
for (const operation of [
  ...
  const binary = `body: ${operation}Body`;
  const occurrences = fixedClient.split(serialized).length - 1;
  if (occurrences !== 1) {
    throw new Error(
      `Expected exactly one Orval binary-body serialization for ${operation}, found ${occurrences}`,
    );
  }
  fixedClient = fixedClient.replace(serialized, binary);
}
await writeFile(generatedClientUrl, fixedClient);
...
for (const name of duplicateQueryTypeExports) {
  ...
  if (duplicateOccurrences !== 1) {
    throw new Error(
      `Expected exactly one duplicate ${name} export, found ${duplicateOccurrences}`,
    );
  }
...
const generatedZodApiUrl = new URL(
...
if (fileSchemaOccurrences !== 2) {
  throw new Error(
    `Expected two FSE File schemas, found ${fileSchemaOccurrences}`,
  );
}
await writeFile(
  generatedZodApiUrl,
...
for (const filename of [
  ...
  const typeUrl = new URL(
    `../../api-zod/src/generated/types/${filename}`,
    import.meta.url,
  );
  ...
  if (blobOccurrences !== 1) {
    throw new Error(
      `Expected one Blob field in ${filename}, found ${blobOccurrences}`,
    );
  }
  await writeFile(
    typeUrl,
    typeSource.replace("file: Blob;", "file: Uint8Array;"),
  );
}
