...
- An aggregate report row keyed on a *visible* entity must still scope the COUNTED records, not just the entity row. A universal mezzo (`centro_ascolto_id NULL`) is visible to every caller, but its consegne/bolle/turni belong to specific centri/Aree Operative.
- **Why:** without scoping each count subquery, an Area Operativa-scoped caller sees usage totals from other Aree Operative through the universal mezzo — an aggregate cross-boundary leak past the HARD Area Operativa boundary.
