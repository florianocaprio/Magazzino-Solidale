...
test.describe("UX-CARICO-01 — bozza e conferma carico", () => {
  test("evidenzia errori, conserva A quando salva B, crea attività e carica solo B", async ({
    ...
    test.skip(
      !["desktop-1440x900", "tablet-820x1180"].includes(testInfo.project.name),
      "Flusso UX verificato su desktop e tablet simulato",
    );
    await login(page);
    ...
    expect(
      ((await detailResponse.json()) as { integrazioni: unknown[] })
        .integrazioni,
    ).toHaveLength(1);
    await expect(
