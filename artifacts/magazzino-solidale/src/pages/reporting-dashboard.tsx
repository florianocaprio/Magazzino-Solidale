...
} from "@/components/reporting/report-filters";
...
export function reportingFiltersFromSearch(
  search: string,
  user: Pick<
  ...
  return {
    da,
    a,
    areaOperativaId: positiveId(
      params,
      "areaOperativaId",
      user?.areaOperativaId ?? null,
    ),
    centroAscoltoId: positiveId(
      params,
      "centroAscoltoId",
...
function initialFilters(
  ...
  return reportingFiltersFromSearch(window.location.search, user);
}
...
function QueryContent({
  ...
  const drilldownParams: Omit<
    GetReportDrilldownParams,
    "page" | "pageSize"
  > | null = metric ? { section, metric, ...apiParams(filters) } : null;
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-sm text-muted-foreground">
          <p>
            {t("reporting.period", {
              from: report.filters.da,
              to: report.filters.a,
            })}
          </p>
          <p>
            {t("reporting.generated", {
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {report.kpi.map((item) => (
            <ReportKpiCard
              ...
              item={item}
              onOpen={
                item.drilldownMetric
                  ? () => setMetric(item.drilldownMetric)
                  : undefined
              }
            />
          ))}
      ...
      {report.series.length > 0 && (
        <section className="grid gap-4 xl:grid-cols-2">
          {report.series.map((series) => (
            <ReportChartCard key={series.key} series={series} />
          ))}
        </section>
      )}
      {report.tables.length > 0 && (
        <section className="grid gap-4 xl:grid-cols-2">
          {report.tables.map((table) => (
            <ReportTable key={table.key} table={table} />
          ))}
        </section>
      )}
...
function ReportLoader({
  ...
  switch (section) {
    case "generale":
      return (
        <GeneralLoader section={section} filters={filters} params={params} />
      );
    case "pacchi":
      return (
        <PacchiLoader section={section} filters={filters} params={params} />
      );
    case "centro-ascolto":
      return (
        <CentroLoader section={section} filters={filters} params={params} />
      );
    case "emporio":
      return (
        <EmporioLoader section={section} filters={filters} params={params} />
      );
    case "mensa":
      return (
        <MensaLoader section={section} filters={filters} params={params} />
      );
    case "uds":
      return <UdsLoader section={section} filters={filters} params={params} />;
    case "magazzino-logistica":
      return (
        <LogisticaLoader section={section} filters={filters} params={params} />
      );
    case "fse-plus":
      return <FseLoader section={section} filters={filters} params={params} />;
  }
}
...
function GeneralLoader(p: LoaderProps) {
  return (
    <QueryContent {...p} query={useGetReportDashboardGenerale(p.params)} />
  );
}
function PacchiLoader(p: LoaderProps) {
  return <QueryContent {...p} query={useGetReportPacchi(p.params)} />;
}
function CentroLoader(p: LoaderProps) {
  return <QueryContent {...p} query={useGetReportCentroAscolto(p.params)} />;
}
function EmporioLoader(p: LoaderProps) {
  return <QueryContent {...p} query={useGetReportEmporio(p.params)} />;
}
function MensaLoader(p: LoaderProps) {
  return <QueryContent {...p} query={useGetReportMensaIntegrato(p.params)} />;
}
function UdsLoader(p: LoaderProps) {
  return <QueryContent {...p} query={useGetReportUdsIntegrato(p.params)} />;
}
function LogisticaLoader(p: LoaderProps) {
  return (
    <QueryContent {...p} query={useGetReportMagazzinoLogistica(p.params)} />
  );
}
function FseLoader(p: LoaderProps) {
  return <QueryContent {...p} query={useGetReportFsePlusIntegrato(p.params)} />;
}
    ...
    <div className="space-y-6 p-4 md:p-8">
      <header>
        <h1 className="text-3xl font-bold tracking-tight">
          {t(`reporting.sections.${section}.title`)}
        </h1>
        <p className="mt-1 text-muted-foreground">
          {t(`reporting.sections.${section}.description`)}
        </p>
      </header>
      <ReportFilters
        section={section}
