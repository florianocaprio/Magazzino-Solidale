...
import {
  ...
  useDeleteConsegna,
  useAssociaBolla,
  useInviaEmailConsegnaBeneficiario,
  useInviaEmailConsegnaVolontario,
  useListBolle,
  useListBeneficiari,
  useGetBeneficiario,
...
export default function Consegne() {
  ...
  const [centroFilter, setCentroFilter] = useState(
    initialSearch.get("centro") ?? "all",
  );
  const [statoFilter, setStatoFilter] = useState(
    initialSearch.get("stato") ?? "all",
  );
  ...
  const [dataInizio, setDataInizio] = useState(
    initialSearch.get("dal") ?? lastMonthRange.inizio,
  );
  const [dataFine, setDataFine] = useState(
    initialSearch.get("al") ?? lastMonthRange.fine,
  );
  ...
  if (centroFilter !== "all")
    consegneParams.centroAscoltoId = parseInt(centroFilter);
  ...
  const { data: beneficiari } = useListBeneficiari({
    attivo: true,
    ...(createCentroId !== "all"
      ? { centroAscoltoId: parseInt(createCentroId) }
      : {}),
  });
  ...
  const { data: areaOperativaList } = useListAreeOperative({
    query: {
      queryKey: getListAreeOperativeQueryKey(),
      enabled: isAreaOperativaGlobal,
    },
  });
  ...
  const areaOperativaNotChosen =
    isAreaOperativaGlobal && areaOperativaFilter === "all";
  const centriFiltrati = (centri ?? []).filter((c) => {
    if (!isAreaOperativaGlobal) return true;
    if (areaOperativaFilter === "all") return false;
    return (
      c.areaOperativaId != null &&
      String(c.areaOperativaId) === areaOperativaFilter
    );
  });
  ...
  const [creatingBollaFor, setCreatingBollaFor] = useState<Consegna | null>(
    null,
  );
  ...
  const handleAssocia = (bollaId: number | null) => {
    if (!associatingId || !associatingConsegna) return;
    const versione =
      bollaId == null
        ? associatingConsegna.bollaVersione
        : (bolle ?? []).find((item) => item.id === bollaId)?.versione;
    if (versione == null) {
      toast({
        title: t("consegne.toastOpFallita"),
        description: t("consegne.toastErrore"),
        variant: "destructive",
      });
      return;
    }
    const slot = `consegna:${associatingId}:associa-bolla:${bollaId ?? "null"}`;
    const semanticInput = { bollaId, versione };
    associaBolla.mutate(
      {
        id: associatingId,
      ...
      },
      {
        onSuccess: () => {
          ...
          toast({
            title: bollaId
              ? t("consegne.toastBollaAssociata")
              : t("consegne.toastBollaScollegata"),
          });
          setAssociatingId(null);
          setSelectedBollaId("");
        },
        onError: (e: unknown) => {
          commandIntents.fail(slot, e);
          queryClient.invalidateQueries({
            queryKey: getListConsegneQueryKey(),
          });
  ...
  const beneficiarioSelezionato = useMemo(
    () =>
      [...(beneficiari ?? []), ...(allBeneficiari ?? [])].find(
  ...
  const effectiveConsegnaCentroId =
    beneficiarioSelezionato?.centroAscoltoId ??
    ...
    () =>
      (volontari ?? []).filter((v, idx, all) => {
        ...
        return (
          ...
          v.centroAscoltoId === effectiveConsegnaCentroId
        );
      }),
    ...
    () =>
      (mezzi ?? []).filter((m, idx, all) => {
        ...
        if (
          ...
          return false;
        if (
          (m.scadenzaAssicurazione != null &&
            m.scadenzaAssicurazione.slice(0, 10) < dataPrevistaWatch) ||
          (m.scadenzaRevisione != null &&
            m.scadenzaRevisione.slice(0, 10) < dataPrevistaWatch)
        )
          return false;
        if (m.effectiveCentroId == null) return true;
        return (
          effectiveConsegnaCentroId != null &&
          m.effectiveCentroId === effectiveConsegnaCentroId
        );
      }),
    ...
    createPendingVolontario.mutate(
      {
        data: {
          ...
          note:
            nuovoVolontario.note.trim() ||
            "Inserito da pianificazione consegne",
        },
      },
      {
        onSuccess: (created) => {
          setPendingVolontari((prev) => [
            created,
          ...
          toast({
            description: t("turni.pendingVolCreated", {
              defaultValue: "Volontario inserito in attesa di approvazione",
            }),
          });
          setNuovoVolontario({
            nome: "",
  ...
  const creaMezzoPending = () => {
    ...
    createPendingMezzo.mutate(
      ...
      {
        onSuccess: (created) => {
          setPendingMezzi((prev) => [
            created,
            ...prev.filter((m) => m.id !== created.id),
          ]);
          queryClient.invalidateQueries({ queryKey: getListMezziQueryKey() });
          toast({
            description: t("turni.pendingMezzoCreated", {
              defaultValue: "Mezzo inserito in attesa di approvazione",
            }),
          });
          setNuovoMezzo({
            tipo: "",
        ...
        onError: (e: unknown) =>
          toast({ variant: "destructive", description: apiErrorMessage(e) }),
      },
    );
  };
  ...
  const handleScan = (codeOverride?: string) => {
    ...
    setScanCode("");
    toast({
      title: t("consegne.scanFound", { name: `${b.cognome} ${b.nome}` }),
    });
  };
  ...
  const handleCompleta = () => {
    ...
    const slot = `consegna:${completingId}:completa`;
    const semanticInput = {
      bollaId: consegna.bollaId,
      versione: consegna.bollaVersione,
    };
    completaConsegna.mutate(
      {
        id: completingId,
        data: commandIntents.prepare(slot, semanticInput, {
          versione: consegna.bollaVersione,
  ...
  const handleInviaEmail = (
    id: number,
    destinatario: "beneficiario" | "volontario",
  ) => {
    const mutation =
      destinatario === "beneficiario"
        ? inviaEmailBeneficiario
        : inviaEmailVolontario;
    mutation.mutate(
      { id },
      {
        onSuccess: (res) => {
          if (res.sent) {
            ...
            toast({
              title: t("consegne.toastEmailErrore"),
  ...
  const exportParams = {
    ...
    ...(centroFilter !== "all"
      ? { centroAscoltoId: Number(centroFilter) }
      : {}),
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
          ...
          {canExport && (
            <ExportButtons
              ...
              columns={[
                ...
                {
                  header: t("consegne.colDataPrevista"),
                  accessor: (c) =>
                    c.dataPrevista
                ...
                {
                  header: t("consegne.colFasciaOraria"),
                  accessor: (c) => c.fasciaOraria,
                },
                {
                  header: t("consegne.beneficiario"),
                  accessor: (c) => c.beneficiarioNome,
                },
                {
                  header: t("common.centro"),
                  accessor: (c) => c.centroAscoltoNome ?? "",
                },
                {
                  header: t("common.type"),
                  accessor: (c) => c.tipoConsegna?.replace("_", " "),
                },
                {
                  header: t("common.address"),
                  accessor: (c) => c.indirizzoConsegna,
                },
                { header: t("consegne.zona"), accessor: (c) => c.zona },
                {
                  header: t("consegne.magazzino"),
                  accessor: (c) => c.magazzinoNome,
                },
                {
                  header: t("consegne.volontario"),
                  accessor: (c) => c.volontarioNome ?? c.volontarioAltro ?? "",
                },
      ...
      <Card>
          ...
          <div className="hidden flex-wrap items-center gap-2 lg:flex">
            ...
            {isGlobal && isAreaOperativaGlobal && (
              <Select
                ...
                onValueChange={handleAreaOperativaFilterChange}
              >
                <SelectTrigger className="w-[180px]">
                  <SelectValue
                    placeholder={t("consegne.filterAreaOperativa")}
                  />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">
                    {t("consegne.filterAllAreaOperativa")}
                  </SelectItem>
            ...
            {isGlobal && (
              <Select
                ...
                disabled={areaOperativaNotChosen}
              >
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="all">
                  {t("consegne.filterAllStatuses")}
                </SelectItem>
                <SelectItem value="pianificata">
                  {t("consegne.statoPianificata")}
                </SelectItem>
                <SelectItem value="effettuata">
                  {t("consegne.statoEffettuata")}
                </SelectItem>
              </SelectContent>
            </Select>
            <Input
        ...
        <CardContent className="p-0">
          <div
            data-testid="consegne-mobile-list"
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {format(
                            new Date(`${c.dataPrevista.slice(0, 10)}T12:00:00`),
                            "dd MMM yyyy",
                            { locale: it },
                          )}
                      ...
                      <Badge
                        variant={
                          c.stato === "effettuata" ? "default" : "outline"
                        }
                        className={
                          c.stato === "effettuata"
                            ? "bg-green-600"
                      ...
                      >
                        {c.stato === "effettuata"
                          ? t("consegne.badgeConsegnata")
                          : t("consegne.badgePianificata")}
                      </Badge>
                    </div>
                    <div className="space-y-1 text-sm">
                      {isGlobal && c.centroAscoltoNome && (
                        <p>{c.centroAscoltoNome}</p>
                      )}
                      ...
                      {c.volontarioId != null &&
                        c.volontarioOperativo === false && (
                          <p className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-2 text-destructive">
                            ...
                            <span>
                              Il volontario assegnato non è più operativo per
                              questa data. Verifica l’assegnazione.
                            </span>
                          </p>
                        )}
                    </div>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      {c.bollaId != null && (
                        <Button
                          ...
                          variant="outline"
                          onClick={() => setViewingBollaId(c.bollaId!)}
                        >
                          <FileText className="h-4 w-4" />{" "}
                          {c.stato === "effettuata"
                            ? t("consegne.btnBolla")
                            : t("consegne.btnCompilaBolla")}
                        </Button>
                      )}
                      {c.stato === "pianificata" &&
                        c.bollaId == null &&
                      ...
                      {c.stato === "pianificata" &&
                        ...
                        canComplete && (
                          <Button
                            className="min-h-11 gap-2 bg-green-600 hover:bg-green-700"
                      ...
                      {c.stato === "pianificata" && canCancel && (
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => setAnnullandoId(c.id)}
                        >
                          <Trash2 className="h-4 w-4" />{" "}
                          {t("consegne.btnAnnulla")}
                        </Button>
                      )}
                    </div>
                  </article>
          ...
          <Table
            data-testid="consegne-desktop-list"
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-1 text-sm">
                          ...
                          <>
                            ...
                            {c.magazzinoNome && (
                              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                ...
                                {t("consegne.preparazionePresso", {
                                  magazzino: c.magazzinoNome,
                                })}
                              </div>
                            )}
                          </>
                        ...
                        {(c.volontarioNome || c.volontarioAltro) && (
                          <div className="text-xs text-muted-foreground">
                            {t("consegne.volontarioPrefix", {
                              name: c.volontarioNome ?? c.volontarioAltro,
                            })}
                          </div>
                        )}
                        {c.volontarioId != null &&
                          c.volontarioOperativo === false && (
                            <div className="flex items-start gap-1 text-xs text-destructive">
                              ...
                              <span>
                                Assegnazione da verificare: volontario non
                                operativo per la data.
                              </span>
                            </div>
                          )}
                      </div>
                      ...
                      {(() => {
                        ...
                        const badge =
                          c.bollaStato == null ? (
                            ...
                            >
                              <FileText className="h-3 w-3" /> {c.bollaNumero} ·{" "}
                              {t("consegne.pronta")}
                            </Badge>
                          ) : (
                            <Badge
                              ...
                              className="gap-1 border-amber-200 bg-amber-50 text-amber-700"
                            >
                              <FileClock className="h-3 w-3" /> {c.bollaNumero}{" "}
                              · {t("consegne.inPreparazione")}
                            </Badge>
                          );
                        return (
                          <button
                            type="button"
                            disabled={
                              (consegnata && c.bollaId == null) ||
                              (!consegnata && !canManage)
                            }
                            onClick={() => {
                              if (consegnata) {
                                if (c.bollaId != null)
                                  setViewingBollaId(c.bollaId);
                              } else {
                                setAssociatingId(c.id);
                                setSelectedBollaId(
                                  c.bollaId ? String(c.bollaId) : "",
                                );
                              }
                            }}
                            ...
                            title={
                              consegnata
                                ? c.bollaId != null
                                  ? t("consegne.titleViewBolla")
                                  : undefined
                                : t("consegne.titleManageBolla")
                            }
                          >
                            {badge}
                          </button>
                      ...
                      {c.stato === "effettuata" ? (
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          canComplete ? (
                            <Button
                              size="sm"
                              className="gap-1 min-h-11 bg-green-600 hover:bg-green-700"
                              onClick={() => setCompletingId(c.id)}
                            >
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button
                                  ...
                                  className="gap-1"
                                  disabled={
                                    inviaEmailBeneficiario.isPending ||
                                    inviaEmailVolontario.isPending
                                  }
                                >
                                  <Mail className="h-3.5 w-3.5" />{" "}
                                  {t("consegne.btnReminder")}{" "}
                              ...
                              <DropdownMenuContent align="end">
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleInviaEmail(c.id, "beneficiario")
                                  }
                                >
                                  <Mail className="h-3.5 w-3.5 mr-2" />{" "}
                                  {t("consegne.reminderBeneficiario")}
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  disabled={c.volontarioId == null}
                                  onClick={() =>
                                    handleInviaEmail(c.id, "volontario")
                                  }
                                >
                                  <Truck className="h-3.5 w-3.5 mr-2" />{" "}
                                  {t("consegne.reminderVolontario")}
                                </DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                          )}
                          {canCancel && (
                            <Button
                              size="sm"
                              variant="ghost"
                              className="gap-1 min-h-11 text-destructive hover:text-destructive"
                              onClick={() => setAnnullandoId(c.id)}
                            >
          ...
          {!isLoading && consegnePage && consegnePage.totalPages > 1 && (
            <div className="flex flex-col gap-3 border-t p-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-muted-foreground">
                {consegnePage.total} consegne · pagina {consegnePage.page} di{" "}
                {consegnePage.totalPages}
              </p>
              <div className="flex gap-2">
                ...
                <Button
                  className="min-h-11"
                  variant="outline"
                  disabled={page >= consegnePage.totalPages}
      ...
      <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
        <SheetContent className="w-full overflow-y-auto sm:max-w-sm">
          <SheetHeader>
            <SheetTitle>Filtri consegne</SheetTitle>
          </SheetHeader>
          <div className="mt-6 space-y-5">
            <div className="space-y-2">
              <Label htmlFor="consegne-mobile-search">Ricerca</Label>
              <Input
                id="consegne-mobile-search"
                className="min-h-11"
                value={search}
                onChange={(event) => {
                  setSearch(event.target.value);
                ...
                placeholder="Codice, beneficiario o indirizzo"
              />
            </div>
            {isGlobal && isAreaOperativaGlobal && (
              <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="consegne-mobile-from">
                  {t("consegne.filterDateFrom")}
                </Label>
                <Input
                  id="consegne-mobile-from"
                  type="date"
                  className="min-h-11"
                  value={dataInizio}
                  onChange={(event) => {
              ...
              <div className="space-y-2">
                <Label htmlFor="consegne-mobile-to">
                  {t("consegne.filterDateTo")}
                </Label>
                <Input
                  id="consegne-mobile-to"
                  type="date"
                  className="min-h-11"
                  value={dataFine}
                  onChange={(event) => {
      ...
      <Sheet
        ...
        onOpenChange={(open) => {
          if (!open) closeViewingBolla();
        }}
      >
        <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{t("consegne.bollaConsegnaTitle")}</SheetTitle>
          </SheetHeader>
          {viewingBollaId !== null && (
            <BollaDettaglio
              ...
              >
                {isGlobal && isAreaOperativaGlobal && (
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger
                        aria-label={t("consegne.filterAreaOperativa")}
                      >
                        <SelectValue
                          placeholder={t("consegne.filterAreaOperativa")}
                        />
                    ...
                    <FormItem>
                      ...
                      <BeneficiarioCombobox
                        ...
                        onChange={(id) => {
                          ...
                          if (form.getValues("tipoConsegna") === "domicilio") {
                            form.setValue("indirizzoConsegna", "", {
                              shouldValidate: true,
                            });
                          }
                        }}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        >
                          <FormControl>
                            <SelectTrigger
                              aria-label={t("consegne.formFascia")}
                            >
                              <SelectValue />
                            </SelectTrigger>
                          </FormControl>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        <FormControl>
                          <SelectTrigger
                            aria-label={t("consegne.formModalita")}
                          >
                            <SelectValue />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <SelectItem value="in_sede">
                            {t("consegne.modInSede")}
                          </SelectItem>
                          <SelectItem value="domicilio">
                            {t("consegne.modDomicilio")}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </FormItem>
                ...
                {form.watch("tipoConsegna") === "domicilio" && (
                  <div className="space-y-4 pt-2 border-t">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t("consegne.formIndirizzo")}</FormLabel>
                          <FormControl>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t("consegne.formZona")}</FormLabel>
                          <FormControl>
                            <Input {...field} />
                          </FormControl>
                        </FormItem>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          >
                            <FormControl>
                              <SelectTrigger
                                aria-label={t("consegne.formVolontario")}
                              >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="altro">
                                {t("consegne.volontarioAltro", {
                                  defaultValue: "Altro",
                                })}
                              </SelectItem>
                              {volontariConsegna.map((v) => {
                                const overLimit =
                                  v.maxConsegneTurno > 0 &&
                                  (caricoMap.get(v.id) ?? 0) >=
                                    v.maxConsegneTurno;
                                return (
                                  <SelectItem
                                    key={v.id}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {canCreateVolontario && (
                        ...
                        >
                          ...
                          {t("turni.addVolontarioNonCensito", {
                            defaultValue: "Nuovo Volontario",
                          })}
                        </Button>
                      )}
                      {canCreateMezzo && (
                        ...
                        >
                          <Plus className="me-1 h-4 w-4" />{" "}
                          {t("turni.addMezzoNonCensito", {
                            defaultValue: "Nuovo Mezzo",
                          })}
                        </Button>
                      )}
                    </div>
                    {pendingVolontari.length > 0 && (
                      <p className="rounded-md border border-amber-500/30 bg-amber-500/5 p-2 text-xs text-amber-700">
                        {pendingVolontari.map(volontarioLabel).join(", ")} ·{" "}
                        {t("turni.pendingNotSelectable", {
                          defaultValue:
                            "In attesa di approvazione, non selezionabile",
                        })}
                      </p>
                    )}
                    {pendingMezzi.length > 0 && (
                      <p className="rounded-md border border-amber-500/30 bg-amber-500/5 p-2 text-xs text-amber-700">
                        {pendingMezzi.map((mezzo) => mezzo.codice).join(", ")} ·{" "}
                        {t("turni.pendingNotSelectable", {
                          defaultValue:
                            "In attesa di approvazione, non selezionabile",
                        })}
                    ...
                    {form.watch("volontarioAltro") !== undefined && (
                      <FormField
                        control={form.control}
                        name="volontarioAltro"
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>
                              {t("consegne.volontarioAltroNote", {
                                defaultValue: "Nominativo e nota",
                              })}
                            </FormLabel>
                            <FormControl>
                              <Input
                                ...
                                value={field.value ?? ""}
                                placeholder={t(
                                  "consegne.volontarioAltroPlaceholder",
                                  {
                                    defaultValue:
                                      "Es. familiare delegato, vicino di casa...",
                                  },
                                )}
                              />
                            </FormControl>
                    ...
                    {(() => {
                      if (form.watch("volontarioAltro") !== undefined)
                        return null;
                      const mezzoVal = form.watch("mezzoAltro")
                        ? "altro"
                        : form.watch("mezzoId")
                          ? String(form.watch("mezzoId"))
                          : "0";
                      const selVol = volontariConsegna.find(
                        (v) => v.id === Number(form.watch("volontarioId")),
                      );
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={(v) => {
                              if (v === "altro") {
                                form.setValue("mezzoId", 0);
                                form.setValue("mezzoAltro", true);
                              } else {
                                form.setValue("mezzoId", Number(v));
                                form.setValue("mezzoAltro", false);
                              }
                            }}
                          >
                            <SelectTrigger>
                              <SelectValue
                                placeholder={t("consegne.mezzoPlaceholder")}
                              />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="0">
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t("turni.pendingVolTitle", { defaultValue: "Nuovo Volontario" })}
            </DialogTitle>
            <DialogDescription>
              {t("turni.pendingVolDesc", {
                defaultValue:
                  "Il volontario sarà inserito in attesa di approvazione Logistica.",
              })}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNuovoVolontario((v) => ({ ...v, nome: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
                <Label>
                  {t("common.surname", { defaultValue: "Cognome" })}
                </Label>
                <Input
                  value={nuovoVolontario.cognome}
                  onChange={(e) =>
                    setNuovoVolontario((v) => ({
                      ...v,
            ...
            <div className="flex items-center justify-between gap-3 rounded-md border px-3 py-2">
              <Label htmlFor="consegne-patente-b">
                {t("volontari.patenteB", { defaultValue: "Patente B" })}
              </Label>
              <Switch
                id="consegne-patente-b"
                checked={nuovoVolontario.patente}
                onCheckedChange={(checked) =>
                  setNuovoVolontario((v) => ({ ...v, patente: checked }))
                }
              />
            </div>
            <div className="space-y-2">
              <Label>
                {t("volontari.ruoloPrincipale", {
                  defaultValue: "Ruolo principale",
                })}
              </Label>
              <Select
                value={
                  nuovoVolontario.ruoloVolontarioId != null
                    ? String(nuovoVolontario.ruoloVolontarioId)
                    : ""
                }
                onValueChange={(value) =>
                  setNuovoVolontario((v) => ({
                    ...v,
                    ruoloVolontarioId: Number(value),
                  }))
                }
              >
                <SelectTrigger>
                  <SelectValue placeholder={t("volontari.valRuolo")} />
                </SelectTrigger>
                <SelectContent>
                  {(ruoliVolontari ?? [])
                    .filter((ruolo) => ruolo.attivo)
                    .map((ruolo) => (
                      <SelectItem key={ruolo.id} value={String(ruolo.id)}>
                        {ruolo.nome}
                      </SelectItem>
                    ))}
                </SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNuovoVolontario((v) => ({ ...v, note: e.target.value }))
                }
              />
            </div>
          </div>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setVolontarioDialogOpen(false)}
            >
              {t("common.cancel")}
            </Button>
            <Button
      ...
      <Dialog open={mezzoDialogOpen} onOpenChange={setMezzoDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t("turni.pendingMezzoTitle", { defaultValue: "Nuovo Mezzo" })}
            </DialogTitle>
            <DialogDescription>
              {t("turni.pendingMezzoDesc", {
                defaultValue:
                  "Il mezzo sarà inserito in attesa di approvazione Logistica.",
              })}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNuovoMezzo((m) => ({ ...m, tipo: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNuovoMezzo((m) => ({ ...m, targa: e.target.value }))
                  }
                />
              </div>
            </div>
            <div className="space-y-2">
              <Label>
                {t("mezzi.proprieta", { defaultValue: "Proprietà" })}
              </Label>
              <Select
                value={nuovoMezzo.proprieta}
                onValueChange={(v) =>
                  setNuovoMezzo((m) => ({ ...m, proprieta: v }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
                ...
                <SelectContent>
                  <SelectItem value="associazione">
                    {t("mezzi.proprietaOpts.associazione", {
                      defaultValue: "Associazione",
                    })}
                  </SelectItem>
                  <SelectItem value="centro">
                    {t("mezzi.proprietaOpts.centro", {
                      defaultValue: "Centro",
                    })}
                  </SelectItem>
                  <SelectItem value="volontario">
                    {t("mezzi.proprietaOpts.volontario", {
                      defaultValue: "Volontario",
                    })}
                  </SelectItem>
                  <SelectItem value="noleggio">
                    {t("mezzi.proprietaOpts.noleggio", {
                      defaultValue: "Noleggio",
                    })}
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              <Label>
                {t("mezzi.descrizione", { defaultValue: "Descrizione" })}
              </Label>
              <Input
                value={nuovoMezzo.descrizione}
                onChange={(e) =>
                  setNuovoMezzo((m) => ({ ...m, descrizione: e.target.value }))
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNuovoMezzo((m) => ({ ...m, note: e.target.value }))
                }
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                createPendingMezzo.isPending ||
                effectiveConsegnaCentroId == null ||
                !nuovoMezzo.tipo.trim()
              }
            >
              {t("common.save")}
            </Button>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("consegne.dialogCompletaTitle")}
            </AlertDialogTitle>
            <AlertDialogDescription>
      ...
      <AlertDialog
        open={annullandoId != null}
        onOpenChange={(open) => !open && setAnnullandoId(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("consegne.annullaTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("consegne.annullaDesc")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t("common.cancel")}</AlertDialogCancel>
            <AlertDialogAction
              ...
              disabled={deleteConsegna.isPending}
      ...
      <AlertDialog
        open={ripianificando != null}
        onOpenChange={(open) => !open && setRipianificando(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("consegne.ripianificaTitle")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("consegne.ripianificaDesc")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          {ripianificando && (
            <div className="py-2 space-y-4">
              <p className="text-sm font-medium">
                {ripianificando.beneficiarioNome}
              </p>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>{t("common.cancel")}</AlertDialogCancel>
            <Button
              disabled={!riDate || createConsegna.isPending}
              onClick={handleRipianifica}
            >
              {t("consegne.btnRipianifica")}
            </Button>
          </AlertDialogFooter>
        ...
        <AlertDialogContent>
          ...
          <div className="py-2 space-y-3">
            {bolleDisponibili.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                {t("consegne.noBollaAvailable")}{" "}
                <span className="font-medium">
                  {t("consegne.bolleSection")}
                </span>
                .
              </p>
            ) : (
              <Select
                value={selectedBollaId}
                onValueChange={setSelectedBollaId}
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={t("consegne.selectBollaPlaceholder")}
                  />
                </SelectTrigger>
                <SelectContent>
                  {bolleDisponibili.map((b) => (
                    <SelectItem key={b.id} value={String(b.id)}>
                      {b.numeroBolla} ·{" "}
                      {b.stato === "confermato"
                        ? t("consegne.optPronta")
                        : b.stato === "consegnato"
                          ? t("consegne.optConsegnata")
                          : t("consegne.optInPreparazione")}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <AlertDialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button
              ...
              onClick={() =>
                handleAssocia(
                  selectedBollaId ? parseInt(selectedBollaId) : null,
                )
              }
            >
