...
export function mapsEntityDeepLink(marker: MapsMarker): string | null {
  if (marker.entityType === "intervento")
  ...
  if (marker.entityType === "centro_ascolto")
    return `/centri-ascolto?centroId=${marker.entityId}`;
  return null;
}

function mapsLayerError(error: unknown, fallback: string): string {
...
export default function MapsOperativa() {
  ...
  const enabled = (code: MapsLayerCode) =>
    available.some((layer) => layer.code === code) && !disabled.has(code);
  const validRange = da <= a;
  const range = { da, a };
  const social = useGetMapsInterventiSociali(range, {
    query: {
      queryKey: getGetMapsInterventiSocialiQueryKey(range),
      enabled: validRange && enabled("sociale.interventi_pianificati"),
    },
  });
  const deliveries = useGetMapsConsegne(range, {
    query: {
      queryKey: getGetMapsConsegneQueryKey(range),
      enabled: validRange && enabled("pacchi.consegne"),
    },
  });
  const missed = useGetMapsRitiriNonEffettuati(range, {
    query: {
      queryKey: getGetMapsRitiriNonEffettuatiQueryKey(range),
      enabled: validRange && enabled("pacchi.ritiri_non_effettuati"),
    },
  });
  const points = useGetMapsPuntiOperativi({
    query: {
      queryKey: getGetMapsPuntiOperativiQueryKey(),
      enabled: enabled("centro.punti_operativi"),
    },
  });
  const markers = useMemo(
    () =>
      [
        enabled("sociale.interventi_pianificati") && validRange
          ? social.data
          : [],
        enabled("pacchi.consegne") && validRange ? deliveries.data : [],
        enabled("pacchi.ritiri_non_effettuati") && validRange
          ? missed.data
          : [],
        enabled("centro.punti_operativi") ? points.data : [],
      ].flatMap((rows) => rows ?? []),
    [
      social.data,
      deliveries.data,
  ...
  const layerErrors = [
    ...
    { code: "pacchi.consegne" as const, query: deliveries },
    { code: "pacchi.ritiri_non_effettuati" as const, query: missed },
  ...
  const isRefreshing =
    social.isRefetching ||
    deliveries.isRefetching ||
    missed.isRefetching ||
    points.isRefetching;

  const toggle = (code: MapsLayerCode) =>
    setDisabled((current) => {
  ...
  const onMapUnavailable = useCallback(
    () =>
      toast({
        title: t("maps.mapUnavailable"),
        description: t("maps.listFallback"),
      }),
    [t, toast],
  );
  const refresh = () =>
    Promise.all(
      [
        enabled("sociale.interventi_pianificati") && validRange
          ? social.refetch()
          : undefined,
        enabled("pacchi.consegne") && validRange
          ? deliveries.refetch()
          : undefined,
        enabled("pacchi.ritiri_non_effettuati") && validRange
          ? missed.refetch()
          : undefined,
        enabled("centro.punti_operativi") ? points.refetch() : undefined,
      ].filter(Boolean),
    );
  const hasPendingLocations = markers.some(
    (marker) => marker.locationStatus === "pending",
  );
  useEffect(() => {
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[minmax(9rem,1fr)_minmax(9rem,1fr)_auto]">
          ...
          <div className="space-y-1">
            <Label htmlFor="maps-a">{t("maps.to")}</Label>
            <Input
              className="min-h-11"
              id="maps-a"
              type="date"
          ...
          <Button
            ...
            disabled={isRefreshing || !validRange}
          >
            <RefreshCw
              className={
            ...
            />
            {t(isRefreshing ? "maps.refreshing" : "maps.refresh")}
          </Button>
          <div className="col-span-full grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {available.map((layer) => (
              ...
              >
                <Switch
                  checked={enabled(layer.code)}
                  onCheckedChange={() => toggle(layer.code)}
                />
                {t(LAYER_LABELS[layer.code])}
              </Label>
            ))}
          </div>
        </CardContent>
      ...
      <LeafletOperationalMap
        ...
        tileUrl={window.__APP_CONFIG__?.mapsTileUrl}
        tileAttribution={window.__APP_CONFIG__?.mapsTileAttribution}
      />
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            <MapPin className="h-4 w-4" />
            {t("maps.operationalList")}
          </CardTitle>
        </CardHeader>
            ...
            markers.map((marker) => (
              ...
              >
                <button
                  ...
                  aria-label={`${t("maps.markerDetails")}: ${marker.title}`}
                  onClick={() => setSelectedMarker(marker)}
                >
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="font-medium">{marker.title}</p>
                    <Badge variant="outline">{marker.status}</Badge>
                  </div>
                  <p className="truncate text-sm text-muted-foreground">
                    {marker.address}
                  </p>
                  {marker.locationStatus !== "resolved" && (
                  ...
                  {marker.date && (
                    <p className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {formatDateOrDateTimeEuropeRome(marker.date)}
                    </p>
                  )}
                </button>
                <div className="flex shrink-0 flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() => setSelectedMarker(marker)}
                  >
                    {t("maps.markerDetails")}
                  </Button>
                  <RouteActions
                    consegnaId={marker.entityId}
                    available={
                      marker.entityType === "consegna" &&
                      marker.actions.includes("route")
                    }
                    compact
                  />
                </div>
      ...
      >
        <SheetContent className="z-[60] w-full overflow-y-auto sm:max-w-md">
          {selectedMarker && (
            <>
              <SheetHeader>
                ...
                <SheetDescription>
                  {selectedMarker.subtitle ??
                    t(LAYER_LABELS[selectedMarker.layer])}
                </SheetDescription>
              </SheetHeader>
              <div className="mt-6 space-y-4">
                ...
                {selectedMarker.date && (
                  <div>
                    ...
                    <p className="text-sm">
                      {formatDateOrDateTimeEuropeRome(selectedMarker.date)}
                    </p>
                  </div>
                )}
                <div>
                ...
                <div className="flex flex-col gap-2 pt-2">
                  {selectedMarker.actions.includes("open") &&
                    mapsEntityDeepLink(selectedMarker) && (
                      <Button asChild className="min-h-11" variant="outline">
                        <Link href={mapsEntityDeepLink(selectedMarker)!}>
                          {t("maps.openOwner")}
                        </Link>
                      </Button>
                    )}
                  <RouteActions
                    consegnaId={selectedMarker.entityId}
                    available={
                      selectedMarker.entityType === "consegna" &&
                      selectedMarker.actions.includes("route")
                    }
                  />
