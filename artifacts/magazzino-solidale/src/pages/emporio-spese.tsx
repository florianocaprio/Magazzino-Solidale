...
import { Copy, Download, Mail, RotateCcw, Search } from "lucide-react";
...
function buildPrintHtml(data: BollaEmporioStampa): string {
  const rows = data.righe
    .map(
      (riga) => `
    <tr><td>${escapeHtml(riga.descrizioneProdotto)}</td><td>${escapeHtml(riga.codiceProdotto ?? "")}</td><td>${escapeHtml(formatQuantita(riga.quantita, riga.unitaMisura))}</td><td>${escapeHtml(formatCredito(riga.creditoUnitario))}</td><td>${escapeHtml(formatCredito(riga.creditoTotale))}</td></tr>
  `,
    )
    .join("");
...
function emailBadgeClass(stato: string): string {
  if (stato === "invio_manuale_avviato")
    return "bg-emerald-500/10 text-emerald-700 border-emerald-200";
  if (stato === "nessun_destinatario")
...
export default function EmporioSpese() {
  ...
  useEffect(() => {
    setPage(1);
  }, [dataDa, dataA, beneficiarioSearch, centroFilter, emporioFilter]);
  ...
  const openStorno = (spesa: SpesaEmporio) => {
    ...
    setQuantitaStorno(
      Object.fromEntries(
        spesa.righe.map((riga) => [riga.id, riga.quantitaStornabile]),
      ),
    );
    setStornoOpen(true);
  };
  ...
  const prepareEmailBolla = async (
    ...
    try {
      ...
      if (openClient && result.mailtoHref) {
        ...
        toast({
          title: t("speseEmporio.emailClientAperto"),
          description: t("speseEmporio.emailClientApertoDescrizione"),
        });
      } else if (openClient) {
        toast({
      ...
      toast({
        title: t("speseEmporio.emailPreparazioneErrore"),
        variant: "destructive",
      });
      return null;
    }
    ...
    <div className="space-y-4 p-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-normal">
          {t("speseEmporio.titolo")}
        </h1>
        <p className="text-sm text-muted-foreground">
          {t("speseEmporio.sottotitolo")}
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 md:grid-cols-5">
          ...
          <Input
            ...
            placeholder={t("speseEmporio.cercaBeneficiario")}
          />
              ...
              <TableBody>
                {spese.map((spesa) => (
                  <TableRow
                    ...
                    onClick={() => setSelectedId(spesa.id)}
                  >
                    ...
                    <TableCell>
                      {formatCredito(spesa.totaleCreditoConsumati)}
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline">{spesa.statoSpesa}</Badge>
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={emailBadgeClass(spesa.emailBollaStato)}
                      >
                        {t(`speseEmporio.email.${spesa.emailBollaStato}`)}
                      </Badge>
                    </TableCell>
                  </TableRow>
                ))}
            ...
            {dettaglio ? (
              <>
                <div className="grid gap-2">
                  <div>
                    <span className="text-muted-foreground">
                      {t("speseEmporio.numeroSpesa")}:{" "}
                    </span>
                    <span className="font-medium">{dettaglio.numeroSpesa}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">
                      {t("speseEmporio.numeroBolla")}:{" "}
                    </span>
                    <span className="font-medium">
                      {dettaglio.bollaNumero ?? "-"}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">
                      {t("speseEmporio.beneficiario")}:{" "}
                    </span>
                    <span className="font-medium">
                      {dettaglio.beneficiarioNome ?? "-"}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">
                      {t("speseEmporio.saldoPrima")}:{" "}
                    </span>
                    <span className="font-medium">
                      {formatCredito(dettaglio.saldoPrima)}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">
                      {t("speseEmporio.saldoDopo")}:{" "}
                    </span>
                    <span className="font-medium">
                      {formatCredito(dettaglio.saldoDopo)}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Stato: </span>
                    <span className="font-medium">{dettaglio.statoSpesa}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">
                      {t("speseEmporio.statoEmailBolla")}:{" "}
                    </span>
                    <span className="font-medium">
                      {t(`speseEmporio.email.${dettaglio.emailBollaStato}`)}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">
                      {t("speseEmporio.emailDataUltimoClick")}:{" "}
                    </span>
                    <span className="font-medium">
                      {formatDateTime(dettaglio.emailBollaDataUltimoClick)}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">
                      {t("speseEmporio.emailOperatore")}:{" "}
                    </span>
                    <span className="font-medium">
                      {dettaglio.emailBollaOperatoreId ?? "-"}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">
                      {t("speseEmporio.emailOggetto")}:{" "}
                    </span>
                    <span className="font-medium">
                      {dettaglio.emailBollaOggetto ??
                ...
                <div className="space-y-2">
                  {dettaglio.righe.map((riga) => (
                    <div key={riga.id} className="rounded-md border p-3">
                      ...
                      <div>
                        {t("speseEmporio.creditoConsumati")}:{" "}
                ...
                <div className="flex flex-col gap-2">
                  <Button
                    variant="outline"
                    onClick={() => printBolla(dettaglio)}
                  >
                    <Download className="mr-2 h-4 w-4" />
                    {t("speseEmporio.stampaBolla")}
                  </Button>
                  {canManage &&
                    (emailDraftBolla?.mailtoHref ? (
                      <Button variant="outline" asChild>
                        ...
                        >
                          ...
                          {t("speseEmporio.ritentaInvioEmail")}
                        </a>
                      </Button>
                    ) : (
                      <Button
                        variant="outline"
                        onClick={() => {
                          void prepareEmailBolla(dettaglio, true);
                        }}
                  ...
                  <Button
                    variant="outline"
                    onClick={() => {
                      void copyBollaLink(dettaglio);
                    }}
                  >
                    <Copy className="mr-2 h-4 w-4" />
                    {t("speseEmporio.copiaLinkBolla")}
                  </Button>
                  {canManage && (
                    <Button
                      variant="outline"
                      onClick={() => {
                        void copyEmailText(dettaglio);
                      }}
                      disabled={registraInvioManualeBolla.isPending}
                    >
                      <Copy className="mr-2 h-4 w-4" />
                      {t("speseEmporio.copiaTestoEmail")}
                    </Button>
                  )}
                  {canReverse && dettaglio.statoSpesa !== "stornata" && (
                    <Button
                      variant="destructive"
          ...
          {dettaglio && (
            <div className="space-y-4 text-sm">
              <div className="rounded-md border p-3">
                <div className="font-medium">
                  Spesa originale {dettaglio.numeroSpesa}
                </div>
                <div>
                  {dettaglio.beneficiarioNome ?? "-"} ·{" "}
