...
export function RouteActions({
  ...
  if (!available || !hasMapsPermission(user, hasPermission, "maps.route"))
    return null;
  ...
  const copyUrl = async (url: string): Promise<boolean> => {
    if (!navigator.clipboard?.writeText) {
      setManualUrl(url);
      toast({
        title: t("maps.manualCopyTitle"),
        description: t("maps.manualCopyDescription"),
      });
      return false;
    }
    try {
  ...
  const buttonClass = compact ? "min-h-11 min-w-11 px-2" : "min-h-11 gap-1";
  return (
    <>
      <div className={cn("flex flex-wrap items-center gap-1", className)}>
        <Button
          ...
          onClick={openRoute}
          aria-label={t("maps.openRoute")}
        >
          <Navigation className="h-3.5 w-3.5" />
          {!compact && t("maps.openRoute")}
        </Button>
        <Button
          type="button"
          size="sm"
          ...
          className={buttonClass}
          onClick={copyRoute}
          aria-label={t("maps.copyRoute")}
        >
          <Copy className="h-3.5 w-3.5" />
          {!compact && t("maps.copyRoute")}
        </Button>
        <Button
          type="button"
          size="sm"
          ...
          className={buttonClass}
          onClick={shareRoute}
          aria-label={t("maps.shareRoute")}
        >
          <Share2 className="h-3.5 w-3.5" />
          {!compact && t("maps.shareRoute")}
        </Button>
      </div>
      <Dialog
        open={manualUrl != null}
        onOpenChange={(open) => {
          if (!open) setManualUrl(null);
        }}
      >
