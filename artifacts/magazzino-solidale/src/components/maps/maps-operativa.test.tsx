    ...
    data: {
      ...
      layers: [
        ...
        ...(apiState.socialDate
          ? [
              {
                code: "sociale.interventi_pianificati",
                domain: "sociale",
                label: "Interventi",
      ...
      : [
          {
            ...
            entityType: "consegna",
            entityId: 1,
            title: "Consegna CON-1",
            subtitle: null,
            status: "pianificata",
            address: "Via Test 1",
            date: apiState.deliveryDate,
            actions: apiState.markerActions,
          },
        ],
    isLoading: false,
    isError: apiState.deliveryError != null,
    error: apiState.deliveryError,
  }),
  useGetMapsInterventiSociali: () => ({
    data: apiState.socialDate
      ? [
          {
            id: "sociale.intervento:2",
            ...
            subtitle: null,
            status: "pianificato",
            address: "Via Sociale 2",
            date: apiState.socialDate,
            actions: ["open"],
          },
        ]
      : undefined,
...
import MapsOperativa, { mapsEntityDeepLink } from "@/pages/maps";
...
describe("MAPS operativa", () => {
  ...
  it("usa l'identità canonica tipo:id per aprire una Bolla", () => {
    expect(
      mapsEntityDeepLink({
        entityType: "bolla",
        entityId: 17,
      } as Parameters<typeof mapsEntityDeepLink>[0]),
    ).toBe("/bolle?documento=bolla%3A17");
  });
  ...
  it("genera i toggle dalle capabilities e mostra la mappa OpenStreetMap senza chiavi", async () => {
    ...
    expect(
      document.querySelector('[aria-label="maps.openRoute"]'),
    ).toBeTruthy();
    expect(document.querySelectorAll('[role="switch"]')).toHaveLength(1);
    expect(
      document.querySelector('[aria-label="Mappa operativa OpenStreetMap"]'),
    ).toBeTruthy();

    const details = Array.from(document.querySelectorAll("button")).find(
      (button) => button.textContent === "maps.markerDetails",
    );
  ...
  it("mostra data e ora Europe/Rome per i marker timestamp", async () => {
    ...
    const details = Array.from(document.querySelectorAll("button")).filter(
      (button) => button.textContent === "maps.markerDetails",
    );
    await act(async () => details[0]?.click());
    expect(
      document.body.textContent?.match(/23\/08\/2026 00:30/g),
    ).toHaveLength(2);
  });
  ...
  it("mantiene gli altri layer utilizzabili e mostra l'errore specifico del layer", async () => {
    apiState.deliveryError = {
      data: { error: "Restringi l'intervallo o i filtri" },
    };
    await act(async () => root.render(<MapsOperativa />));
    expect(document.body.textContent).toContain(
      "Restringi l'intervallo o i filtri",
    );
