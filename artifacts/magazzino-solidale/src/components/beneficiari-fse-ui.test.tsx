...
describe("Beneficiari 2.0 FSE+: UI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.preview.mockResolvedValue({
      ...
      righe: [
        ...
        {
          numeroRiga: 3,
          codiceFascicolo: "FSE-BAD",
          classificazione: "errore",
          errori: ["Dato non valido"],
          warning: [],
        },
      ],
    });
  ...
  it("mostra lo scope derivato e consente un import parziale esplicito", async () => {
    ...
    const input = document.querySelector<HTMLInputElement>(
      'input[aria-label="File FSE+"]',
    )!;
    const bytes = workbookBytes();
    const file = new File([bytes], "beneficiari-test.xlsx", {
      type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    });
    ...
    expect(document.body.textContent).toContain(
      "Le righe non valide saranno escluse",
    );
  ...
  it("visualizza differenze demografiche e limita la modifica al permesso", async () => {
    await act(async () =>
      root.render(
        <BeneficiarioFseCard beneficiarioId={42} canManage={false} />,
      ),
    );
    expect(document.body.textContent).toContain("FSE-CARD");
    expect(document.body.textContent).toContain("Non allineato");
    expect(document.body.textContent).toContain(
      "Differenze: Donne 1→2, Uomini 1→0",
    );
    expect(button("Modifica")).toBeUndefined();

    await act(async () =>
      root.render(<BeneficiarioFseCard beneficiarioId={42} canManage />),
    );
  ...
  it("distingue i valori FSE non valorizzati dallo zero e salva il profilo senza codice", async () => {
    ...
    expect(document.body.textContent).toContain(
      "Origine/minoranze: Non valorizzato",
    );
    expect(document.body.textContent).toContain("Paesi terzi: 0");
    expect(document.body.textContent).toContain(
      "Esclusione abitativa: Non valorizzato",
    );

    await act(async () => button("Modifica")?.click());
    const code = document.querySelector<HTMLInputElement>("#fse-code")!;
    const unknown = document.querySelector<HTMLInputElement>(
      "#fse-origineStranieraMinoranze",
    )!;
    const explicitZero = document.querySelector<HTMLInputElement>(
      "#fse-cittadiniPaesiTerzi",
    )!;
  ...
  it("mantiene null i valori FSE di un beneficiario ancora privo di profilo", async () => {
    ...
    expect(
      document.querySelector<HTMLInputElement>("#fse-origineStranieraMinoranze")
        ?.value,
    ).toBe("");
    expect(
      document.querySelector<HTMLInputElement>("#fse-cittadiniPaesiTerzi")
        ?.value,
    ).toBe("");
    expect(
      document.querySelector<HTMLInputElement>(
        "#fse-senzaTettoEsclusioneAbitativa",
      )?.value,
    ).toBe("");
  ...
  it("richiede una scelta collega o crea e mantiene bloccante il conflitto", async () => {
    ...
    expect(
      (button("Conferma importazione") as HTMLButtonElement).disabled,
    ).toBe(true);
    ...
    await act(async () => {
      button("Conferma importazione")?.click();
      await Promise.resolve();
    });
    expect(mocks.importRows).toHaveBeenCalledWith({
      data: {
    ...
    mocks.preview.mockResolvedValueOnce({
      ...
      righe: [
        {
          numeroRiga: 2,
          codiceFascicolo: "FSE-CONFLICT",
          classificazione: "conflitto",
          errori: ["Codice associato altrove"],
          warning: [],
        },
      ],
    });
    await act(async () => button("Importa FSE+")?.click());
    const conflictInput = document.querySelector<HTMLInputElement>(
      'input[aria-label="File FSE+"]',
    )!;
    const conflictFile = new File([bytes], "conflitto.xlsx");
    Object.defineProperty(conflictFile, "arrayBuffer", {
      value: vi.fn().mockResolvedValue(bytes),
    });
    Object.defineProperty(conflictInput, "files", {
      configurable: true,
      value: [conflictFile],
    });
    await act(async () => {
      conflictInput.dispatchEvent(new Event("change", { bubbles: true }));
      await Promise.resolve();
      await Promise.resolve();
    });
