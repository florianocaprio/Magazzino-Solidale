...
export const DEMO_USER_EMAIL = "operatore.demo@example.org";
...
function assertDemoMarker(label: string, value: string | null | undefined): void {
  if (!value?.includes(DEMO_MARKER)) {
    throw new Error(`Operazione demo annullata: ${label} esiste senza il marcatore ${DEMO_MARKER}.`);
  }
}
...
function demoPassword(): string {
  ...
  if (!value || value.length < 12) {
    throw new Error("DEMO_USER_INITIAL_PASSWORD deve contenere almeno 12 caratteri.");
  }
  return value;
}
...
async function insertAudit(tx: Transaction, actorUserId: number, key: string, summary: EnvironmentDataSummary, note: string): Promise<void> {
...
export async function seedDemoEnvironmentData(actorUserId: number): Promise<EnvironmentDataSummary> {
  ...
  const socialSummary = await db.transaction(async (tx) => {
    ...
    if (demoUser && (demoUser.matricola !== DEMO_USER_MATRICOLA || demoUser.isSuperAdmin || (demoUser.email != null && demoUser.email !== DEMO_USER_EMAIL))) {
      throw new Error(`Seed demo annullato: username ${DEMO_USER_USERNAME} già in uso.`);
    }
    if (demoUser && !demoUser.email) {
      await tx.update(utentiTable).set({ email: DEMO_USER_EMAIL, emailDaAggiornare: false }).where(eq(utentiTable.id, demoUser.id));
      demoUser.email = DEMO_USER_EMAIL;
    }
    if (!demoUser) {
      [demoUser] = await tx
        .insert(utentiTable)
    ...
    let [beneficiary] = await tx.select({ id: beneficiariTable.id, note: beneficiariTable.noteInterne }).from(beneficiariTable).where(eq(beneficiariTable.codice, DEMO_BENEFICIARY_CODE));
    ...
    const [creditMovement] = await tx
      ...
      .where(and(eq(creditoSolidaleMovimentiTable.beneficiarioId, beneficiary.id), eq(creditoSolidaleMovimentiTable.riferimentoTipo, "seed_demo"), like(creditoSolidaleMovimentiTable.note, `%${DEMO_MARKER}%`)));
    if (!creditMovement) {
    ...
    const [udsIntervention] = await tx
      ...
      .where(and(eq(interventiTable.beneficiarioId, beneficiary.id), like(interventiTable.noteUds, `%${DEMO_MARKER}%`)));
    if (!udsIntervention) {
    ...
    const [access] = await tx.select({ id: consegneTable.id, note: consegneTable.noteAccessoEmporio }).from(consegneTable).where(eq(consegneTable.codice, DEMO_ACCESS_CODE));
    ...
    await insertAudit(tx, actorUserId, "seed_demo_ambiente", summary, "Seed sintetico ambiente, UDS ed Emporio; nessun dato reale.");
    return summary;
  });
...
async function demoBeneficiaryIds(tx: Transaction): Promise<number[]> {
  const rows = await tx
    ...
    .where(and(eq(beneficiariTable.codice, DEMO_BENEFICIARY_CODE), like(beneficiariTable.noteInterne, `%${DEMO_MARKER}%`)));
...
async function demoAccessIds(tx: Transaction): Promise<number[]> {
  const rows = await tx
    ...
    .from(consegneTable)
    .where(and(eq(consegneTable.codice, DEMO_ACCESS_CODE), like(consegneTable.noteAccessoEmporio, `%${DEMO_MARKER}%`)));
  return rows.map((row) => row.id);
}

async function deleteDemoEmporio(tx: Transaction): Promise<EnvironmentDataSummary> {
  const accessIds = await demoAccessIds(tx);
  const beneficiaryIds = await demoBeneficiaryIds(tx);
  const sessions = accessIds.length ? await tx.select({ id: sessioniCassaEmporioTable.id }).from(sessioniCassaEmporioTable).where(inArray(sessioniCassaEmporioTable.accessoEmporioId, accessIds)) : [];
  const expenses = accessIds.length ? await tx.select({ id: speseEmporioTable.id }).from(speseEmporioTable).where(inArray(speseEmporioTable.accessoEmporioId, accessIds)) : [];
  if (sessions.length > 0 || expenses.length > 0) {
    throw new Error("Reset demo Emporio annullato: esistono sessioni/spese di cassa. Usare il reset operativo protetto dopo backup.");
  }

  const deletedAccesses = accessIds.length ? await tx.delete(consegneTable).where(inArray(consegneTable.id, accessIds)).returning({ id: consegneTable.id }) : [];
  const deletedCreditMovements = beneficiaryIds.length
    ? await tx
        .delete(creditoSolidaleMovimentiTable)
        .where(and(inArray(creditoSolidaleMovimentiTable.beneficiarioId, beneficiaryIds), like(creditoSolidaleMovimentiTable.note, `%${DEMO_MARKER}%`)))
...
async function deleteDemoUds(tx: Transaction): Promise<EnvironmentDataSummary> {
  ...
  const deletedInterventions = beneficiaryIds.length
    ? await tx
        ...
        .where(and(inArray(interventiTable.beneficiarioId, beneficiaryIds), like(interventiTable.noteUds, `%${DEMO_MARKER}%`)))
        .returning({ id: interventiTable.id })
    : [];
  return { deletedUdsInterventions: deletedInterventions.length };
}

async function deleteDemoBeneficiaries(tx: Transaction): Promise<EnvironmentDataSummary> {
  const beneficiaryIds = await demoBeneficiaryIds(tx);
  if (beneficiaryIds.length === 0) return { deletedBeneficiaries: 0 };

  const references = [await tx.select({ id: consegneTable.id }).from(consegneTable).where(inArray(consegneTable.beneficiarioId, beneficiaryIds)).limit(1), await tx.select({ id: interventiTable.id }).from(interventiTable).where(inArray(interventiTable.beneficiarioId, beneficiaryIds)).limit(1), await tx.select({ id: creditoSolidaleMovimentiTable.id }).from(creditoSolidaleMovimentiTable).where(inArray(creditoSolidaleMovimentiTable.beneficiarioId, beneficiaryIds)).limit(1), await tx.select({ id: bolleTable.id }).from(bolleTable).where(inArray(bolleTable.beneficiarioId, beneficiaryIds)).limit(1), await tx.select({ id: sessioniCassaEmporioTable.id }).from(sessioniCassaEmporioTable).where(inArray(sessioniCassaEmporioTable.beneficiarioId, beneficiaryIds)).limit(1), await tx.select({ id: speseEmporioTable.id }).from(speseEmporioTable).where(inArray(speseEmporioTable.beneficiarioId, beneficiaryIds)).limit(1)];
  if (references.some((rows) => rows.length > 0)) {
    throw new Error("Reset beneficiari demo annullato: rimuovere prima i dati demo Emporio e UDS collegati.");
  }

  const deletedHousehold = await tx.delete(nucleoFamiliareTable).where(inArray(nucleoFamiliareTable.beneficiarioId, beneficiaryIds)).returning({ id: nucleoFamiliareTable.id });
  const deletedBeneficiaries = await tx.delete(beneficiariTable).where(inArray(beneficiariTable.id, beneficiaryIds)).returning({ id: beneficiariTable.id });
  return {
...
async function deleteDemoIdentity(tx: Transaction): Promise<EnvironmentDataSummary> {
  const [user] = await tx
    .select({ id: utentiTable.id, isSuperAdmin: utentiTable.isSuperAdmin })
    .from(utentiTable)
    .where(and(eq(utentiTable.username, DEMO_USER_USERNAME), eq(utentiTable.matricola, DEMO_USER_MATRICOLA)));
  if (user?.isSuperAdmin) {
    throw new Error("Reset demo annullato: l'account demo risulta Super Admin.");
  }
  ...
  if (user) {
    ...
    const users = await tx.delete(utentiTable).where(eq(utentiTable.id, user.id)).returning({ id: utentiTable.id });
  ...
  const [zone] = await tx
    ...
    .from(zoneUdsTable)
    .where(and(eq(zoneUdsTable.nome, DEMO_ZONE_NAME), like(zoneUdsTable.note, `%${DEMO_MARKER}%`)));
  let deletedZones = 0;
  if (zone) {
    const [beneficiaryReference] = await tx.select({ id: beneficiariTable.id }).from(beneficiariTable).where(eq(beneficiariTable.zonaUdsId, zone.id)).limit(1);
    const [userReference] = await tx.select({ id: utentiTable.id }).from(utentiTable).where(eq(utentiTable.zonaUdsId, zone.id)).limit(1);
    if (!beneficiaryReference && !userReference) {
      deletedZones = (await tx.delete(zoneUdsTable).where(eq(zoneUdsTable.id, zone.id)).returning({ id: zoneUdsTable.id })).length;
    }
  }
...
export async function previewDemoReset(): Promise<EnvironmentDataSummary> {
  return db.transaction(async (tx) => {
    ...
    const centres = await tx
      ...
      .where(and(eq(centriAscoltoTable.nome, DEMO_CENTRO_NAME), like(centriAscoltoTable.note, `%${DEMO_MARKER}%`)));
...
export async function resetDemoEmporioData(actorUserId: number): Promise<EnvironmentDataSummary> {
  return db.transaction(async (tx) => {
    const summary = await deleteDemoEmporio(tx);
    await insertAudit(tx, actorUserId, "reset_demo_emporio", summary, "Reset limitato ad accesso e credito Emporio marcati demo.");
    return summary;
  });
}

export async function resetDemoUdsData(actorUserId: number): Promise<EnvironmentDataSummary> {
  return db.transaction(async (tx) => {
    const summary = await deleteDemoUds(tx);
    await insertAudit(tx, actorUserId, "reset_demo_uds", summary, "Reset limitato agli interventi UDS marcati demo.");
    return summary;
  });
}

export async function resetDemoBeneficiaryData(actorUserId: number): Promise<EnvironmentDataSummary> {
  return db.transaction(async (tx) => {
    const summary = await deleteDemoBeneficiaries(tx);
    await insertAudit(tx, actorUserId, "reset_demo_beneficiari", summary, "Reset limitato ai beneficiari con codice e marcatore demo.");
    return summary;
  });
}

export async function resetAllDemoData(actorUserId: number): Promise<EnvironmentDataSummary> {
  const socialSummary = await db.transaction(async (tx) => {
    ...
    await insertAudit(tx, actorUserId, "reset_demo_ambiente", summary, "Reset esclusivo dei record sintetici sociali, UDS ed Emporio.");
    return summary;
  });
