...
export function addCalendarMonthsClamped(date: string, months: number): string {
  if (!Number.isSafeInteger(months)) throw new Error("Durata in mesi non valida");
...
export function inclusiveCoverageEnd(start: string, months: number): string {
  if (!Number.isSafeInteger(months) || months <= 0) throw new Error("Durata in mesi non valida");
...
export function extendedCoverageEnd(currentEnd: string, months: number): string {
  if (!Number.isSafeInteger(months) || months <= 0) throw new Error("Durata in mesi non valida");
...
export function parseFullName(value: unknown): {
  ...
  if (!raw) return { nome: null, cognome: null, warning: "Cognome e Nome mancante" };
  if (raw.includes(",")) {
    ...
    return {
      ...
      warning: nome && cognome ? null : "Separazione del nominativo da verificare",
    };
  }
  const tokens = raw.split(" ");
  if (tokens.length < 2) return { nome: null, cognome: raw, warning: "Nome mancante o non separabile" };
  ...
  return {
    ...
    warning: tokens.length > 2 ? "Verificare la separazione tra cognome e nome" : null,
  };
}
...
export function evaluateOperationalState(input: {
  ...
  riferimento: string;
  coperture: Array<{ dataInizio: string | null; dataFine: string; annullata: boolean }>;
  ...
  const current = coperture.find(
    (item) => (item.dataInizio == null || item.dataInizio <= input.riferimento) && item.dataFine >= input.riferimento,
  );
  const latest = coperture[0] ?? null;
  const future = coperture.find((item) => item.dataInizio != null && item.dataInizio > input.riferimento);
  let statoAssicurazione: InsuranceState;
  if (current) {
    const warningEnd = addCalendarDays(input.riferimento, 30);
    statoAssicurazione = current.dataFine <= warningEnd ? "IN_SCADENZA" : "VALIDA";
  ...
  const base = {
    statoAssicurazione,
    scadenzaAssicurazione: current?.dataFine ?? latest?.dataFine ?? future?.dataFine ?? null,
    sospesoManualmente: !input.amministrativamenteAttivo,
    giornataTemporaneaValida: input.tipoVolontario === "TEMPORANEO" ? input.giornataValida : null,
  };
  if (input.approvazione !== "approvato") {
    return { ...base, operativo: false, motivoNonOperativo: input.approvazione === "respinto" ? "APPROVAZIONE_RESPINTA" : "IN_ATTESA_APPROVAZIONE" };
  }
  if (!input.amministrativamenteAttivo) {
    return { ...base, operativo: false, motivoNonOperativo: "SOSPENSIONE_MANUALE" };
  }
  if (!current) {
    const reason = statoAssicurazione === "SCADUTA"
  ...
  if (input.tipoVolontario === "TEMPORANEO" && !input.giornataValida) {
    return { ...base, operativo: false, motivoNonOperativo: "GIORNATA_TEMPORANEA_MANCANTE" };
  }
