...
const BOLLA_PRIORITA: Record<string, number> = {
  consegnato: 3,
  confermato: 2,
  bozza: 1,
};
...
function assertConsegnaAddress(input: {
  tipoConsegna?: unknown;
  indirizzoConsegna?: unknown;
}): void {
...
async function canAccessConsegna(
  ...
  return (
    canAccessCentro(centroId, callerCentroId(req)) &&
...
async function bollePerConsegne(consegnaIds: number[]) {
  ...
  for (const r of rows) {
    ...
    if (
      !current ||
      (BOLLA_PRIORITA[r.stato] ?? 0) > (BOLLA_PRIORITA[current.stato] ?? 0)
    ) {
      map.set(r.consegnaId, {
        id: r.id,
...
async function loadConsegne(req: Request, options: ConsegneListOptions) {
  ...
  let query = db
    .select({
      ...
      volStatoApprovazione: volontariTable.statoApprovazione,
      volAttivo: volontariTable.attivo,
      volTipo: volontariTable.tipoVolontario,
    })
    .from(consegneTable)
    .leftJoin(
      beneficiariTable,
  ...
  if (options) {
    query = query
      ...
      .offset((options.page - 1) * options.pageSize) as typeof query;
  }
  const rows = await query;

  const operationalByAssignment = new Map<
    string,
    Awaited<ReturnType<typeof operationalStatesForRows>> extends Map<
      number,
      infer T
    >
      ? T
      : never
  >();
  const volunteerRowsByDate = new Map<
    string,
    Array<{
      id: number;
  ...
  for (const row of rows) {
    if (
      row.c.volontarioId == null ||
      row.volStatoApprovazione == null ||
      row.volAttivo == null ||
      row.volTipo == null
    )
      continue;
    const current = volunteerRowsByDate.get(row.c.dataPrevista) ?? [];
    if (!current.some((item) => item.id === row.c.volontarioId)) {
      current.push({
        id: row.c.volontarioId,
        statoApprovazione: row.volStatoApprovazione,
        attivo: row.volAttivo,
        tipoVolontario: row.volTipo,
      });
      volunteerRowsByDate.set(row.c.dataPrevista, current);
    }
  }
  await Promise.all(
    [...volunteerRowsByDate.entries()].map(async ([date, volunteers]) => {
      const states = await operationalStatesForRows(db, volunteers, date);
      for (const [volunteerId, state] of states) {
        operationalByAssignment.set(`${volunteerId}|${date}`, state);
      }
    }),
  );
  ...
  const effectiveCentri =
    effectiveCentroIds.length === 0
      ? []
      : await db
          .select({ id: centriAscoltoTable.id, nome: centriAscoltoTable.nome })
          .from(centriAscoltoTable)
          .where(inArray(centriAscoltoTable.id, effectiveCentroIds));
  const centroNames = new Map(
    effectiveCentri.map((centro) => [centro.id, centro.nome]),
  );
  ...
  const items = rows.map((r) => {
    ...
    const volunteerState =
      r.c.volontarioId == null
        ? null
        : (operationalByAssignment.get(
            `${r.c.volontarioId}|${r.c.dataPrevista}`,
          ) ?? null);
    return {
      id: r.c.id,
      codice: r.c.codice,
      ...
      centroAscoltoNome:
        effectiveCentroId == null
          ? null
          : (centroNames.get(effectiveCentroId) ?? null),
      volontarioId: r.c.volontarioId ?? null,
      volontarioNome:
        r.volNome && r.volCognome ? `${r.volCognome} ${r.volNome}` : null,
      volontarioOperativo: volunteerState?.operativo ?? null,
      volontarioMotivoNonOperativo: volunteerState?.motivoNonOperativo ?? null,
...
router.get(
  ...
  async (req, res) => {
    ...
    const pageSize =
      req.query.pageSize == null ? 25 : Number(req.query.pageSize);
    if (
...
router.get(
  "/consegne/export",
  requirePermission("consegne.export"),
  async (req, res) => {
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      (caller != null || cid != null) &&
      body.magazzinoId != null &&
    ...
    try {
      const row = await db.transaction(async (tx) => {
        ...
        const [created] = await tx
          ...
          } as typeof consegneTable.$inferInsert)
          .returning();
      ...
      res
        .status(201)
        .json({ ...row, dataCreazione: row.dataCreazione.toISOString() });
    } catch (error) {
...
router.get(
  ...
  async (req, res) => {
    const [row] = await db
      .select()
      .from(consegneTable)
      .where(eq(consegneTable.id, Number(req.params.id)));
    if (!row) {
      res.status(404).json({ error: "Not found" });
      return;
    }
    if (row.tipoPianificazione !== TIPO_CONSEGNA_PACCO) {
      res.status(404).json({ error: "Not found" });
      return;
    }
...
router.patch(
  ...
  async (req, res) => {
    ...
    if (existing.tipoPianificazione !== TIPO_CONSEGNA_PACCO) {
      res.status(404).json({ error: "Not found" });
      return;
    }
    const caller = callerCentroId(req);
    ...
    if (
      (caller != null || cid != null || zid != null) &&
      req.body.beneficiarioId != null &&
      req.body.beneficiarioId !== existing.beneficiarioId &&
    ...
    if (
      (caller != null || cid != null) &&
      req.body.magazzinoId != null &&
      req.body.magazzinoId !== existing.magazzinoId &&
    ...
    try {
      const row = await db.transaction(async (tx) => {
        ...
        await validateConsegnaPlanningTx(tx, next, {
          excludeConsegnaId: id,
          context: planning.nuovo ?? undefined,
        });
...
router.delete(
  "/consegne/:id",
  requirePermission("consegne.cancel"),
  async (req, res) => {
    const id = Number(req.params.id);
    const [existing] = await db
      .select()
    ...
    if (existing.stato !== "pianificata") {
      res
        .status(409)
        .json({ error: "Una consegna conclusa non può essere eliminata" });
      return;
    }
    try {
...
router.post(
  ...
  async (req, res) => {
    ...
    if (consegna.tipoPianificazione !== TIPO_CONSEGNA_PACCO) {
      res.status(404).json({ error: "Consegna non trovata" });
      return;
    }
...
router.post(
  "/consegne/:id/completa",
  requirePermission("consegne.complete"),
  async (req, res) => {
    const consegnaId = Number(req.params.id);
...
function luogoConsegna(
  r: NonNullable<Awaited<ReturnType<typeof caricaConsegnaPerEmail>>>,
): string {
  if (r.c.tipoConsegna === "domicilio") {
    return r.c.indirizzoConsegna?.trim() || "Domicilio del beneficiario";
  }
  return (
    [r.magazzinoNome, r.magazzinoIndirizzo].filter(Boolean).join(" — ") ||
    "Magazzino"
  );
}

async function inviaReminderConsegna(
  ...
  const benNomeCompleto =
    r.benNome && r.benCognome
      ? `${r.benCognome} ${r.benNome}`
      : "il beneficiario";
  const volNomeCompleto =
    r.volNome && r.volCognome ? `${r.volCognome} ${r.volNome}` : null;
  ...
  if (destinatario === "beneficiario") {
    ...
    if (!to) {
      res.json({
        sent: false,
        error: "Il beneficiario non ha un indirizzo email",
      });
      return;
    }
    ...
    if (r.c.volontarioId == null) {
      res.json({
        sent: false,
        error: "Nessun volontario assegnato a questa consegna",
      });
      return;
    }
    if (!to) {
      res.json({
        sent: false,
        error: "Il volontario non ha un indirizzo email",
      });
      return;
    }
...
router.post(
  "/consegne/:id/invia-email-beneficiario",
  requirePermission("consegne.manage"),
  (req, res) => inviaReminderConsegna(req, res, "beneficiario"),
);
router.post(
  "/consegne/:id/invia-email-volontario",
  requirePermission("consegne.manage"),
  (req, res) => inviaReminderConsegna(req, res, "volontario"),
);
