...
async function getScaricoWithRighe(id: number) {
  ...
  return {
    ...
    righe: righe.map((r) => {
      const split = provenance.get(`${s.s.codice}:${r.r.prodottoId}`) ?? {
        fse: 0,
        nonFse: 0,
      };
      return {
...
router.get(
  ...
  async (req, res) => {
    ...
    const areaOperativaMagazzini = await visibleMagazzinoIds(
      null,
    ...
    if (operatoreIds.length > 0) {
      ...
      for (const u of utenti)
        opMap.set(u.id, u.matricola ?? u.username ?? null);
    }
    ...
    if (ids.length > 0) {
      ...
      for (const x of righe) {
        ...
        const split = provenance.get(
          `${codiceByScarico.get(x.r.scaricoId)}:${x.r.prodottoId}`,
        ) ?? { fse: 0, nonFse: 0 };
        arr.push({
          id: x.r.id,
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      body.causale === "altro" &&
      (typeof body.causaleAltro !== "string" || !body.causaleAltro.trim())
    ) {
    ...
    if (body.causale === "consegna_beneficiario") {
      ...
      if (
        typeof body.canaleOperativo !== "string" ||
        !["PACCHI", "RITIRO_SEDE", "DOMICILIARE"].includes(
          body.canaleOperativo,
        ) ||
      ...
      if (!beneficiario) {
        res.status(404).json({ error: "Beneficiario non trovato" });
        return;
      }
      ...
      newId = await withDocumentCodeRetry("SCAR", (codice) =>
        db.transaction(async (tx) => {
          ...
          return creaScaricoInventariale(tx, {
            ...
            lottoPolicy,
            source:
              body.causale === "consegna_beneficiario"
                ? {
            ...
            operazioneDistribuzione:
              body.causale === "consegna_beneficiario" && canaleOperativo
                ? {
                    canaleOperativo,
                    dominioOrigine: "MAGAZZINO",
                    entitaOrigineTipo: "scarico_manual_beneficiario",
