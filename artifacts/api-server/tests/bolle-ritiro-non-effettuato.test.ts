...
describe("Bolle — ritiro non effettuato e conversione", () => {
  it("registra un esito strutturato senza cambiare stato o prenotazioni", async () => {
    ...
    const lotto = await createLotto(scope, {
      prodottoId: prodotto,
      magazzinoId: warehouse,
      quantita: 12,
    });
    const riga = await insertBollaRiga(scope, {
      bollaId: bolla,
      prodottoId: prodotto,
      lottoId: lotto,
      quantita: 2,
    ...
    const before = await db
      ...
      .where(eq(prenotazioniMagazzinoTable.bollaId, bolla));
    const [lottoBefore] = await db
      .select()
      .from(lottiTable)
      .where(eq(lottiTable.id, lotto));
    const response = await request(
      makeScopedApp(bolleRouter, { id: operator, centroAscoltoId: centre }),
    )
      .post(`/bolle/${bolla}/ritiro-non-effettuato`)
      .send({ motivo: "Assente" });
    expect(response.status).toBe(200);
    expect(response.body).toMatchObject({
      stato: "confermato",
      ritiroNonEffettuatoMotivo: "Assente",
      ritiroNonEffettuatoOperatoreId: operator,
    });
    ...
    const converted = await request(
      ...
      .send({
        indirizzoConsegna: "Via Stock Invariato 1",
  ...
  it("nega esito su bolla consegnata e a un altro centro", async () => {
    ...
    const appA = makeScopedApp(bolleRouter, {
      id: operator,
    ...
    expect(
      (
        await request(appA)
          .post(`/bolle/${delivered}/ritiro-non-effettuato`)
          .send({})
      ).status,
    ).toBe(409);
    expect(
  ...
  it("converte in una sola consegna con snapshot anche con richieste concorrenti", async () => {
    ...
    await db
      ...
      .set({ domicilio: "Via Domicilio Vecchio 1" })
    ...
    const app = makeScopedApp(bolleRouter, {
      id: operator,
      centroAscoltoId: centre,
    });
    const payload = {
      indirizzoConsegna: "Via Snapshot 9",
      dataPrevista: "2026-09-01",
      fasciaOraria: "Mattina",
      volontarioId: volunteer,
      mezzoId: vehicle,
    };
    ...
    const deliveries = await db
      .select()
      .from(consegneTable)
      .where(eq(consegneTable.id, first.body.consegnaId));
    ...
    const [updatedBolla] = await db
      .select()
      .from(bolleTable)
      .where(eq(bolleTable.id, bolla));
    expect(updatedBolla).toMatchObject({
      consegnaId: first.body.consegnaId,
      indirizzoConsegna: "Via Snapshot 9",
      stato: "confermato",
    });
    ...
    const assignments = await db
      ...
      .where(eq(turniVolontariTable.turnoId, turns[0].id));
  ...
  it("esegue consegna, collegamento bolla e sync turno nella stessa transazione e converge al retry", async () => {
    ...
    const conflictingTurn = await insertTurno(scope, {
      centroAscoltoId: centre,
      data: date,
      fascia: "09-13",
      mezzoId: oldVehicle,
    ...
    const payload = {
      indirizzoConsegna: "Via Atomica 2",
      dataPrevista: date,
      fasciaOraria: "Mattina",
      volontarioId: volunteer,
      mezzoId: requestedVehicle,
    };
    ...
    expect(
      await db
        .select()
        .from(consegneTable)
        .where(eq(consegneTable.beneficiarioId, beneficiary)),
    ).toHaveLength(0);
    expect(
      (await db.select().from(bolleTable).where(eq(bolleTable.id, bolla)))[0]
        .consegnaId,
    ).toBeNull();

    await db
      .update(turniTable)
      .set({ mezzoId: requestedVehicle })
      .where(eq(turniTable.id, conflictingTurn));
    ...
    const assignments = await db
      .select()
      .from(turniVolontariTable)
      .where(eq(turniVolontariTable.turnoId, conflictingTurn));
    expect(assignments).toHaveLength(1);
    expect(assignments[0].volontarioId).toBe(volunteer);
  });
  ...
  it("ripara al retry una conversione storica già collegata ma priva di turno", async () => {
    ...
    const turns = await db
      ...
      .where(
        and(
          eq(turniTable.centroAscoltoId, centre),
          eq(turniTable.data, "2026-09-03"),
          eq(turniTable.fascia, "14-18"),
        ),
      );
    expect(turns).toHaveLength(1);
    scope.turnoIds.push(turns[0].id);
  ...
  it("applica semantica rigorosa a mezzoId, fascia e mezzoAltro", async () => {
    ...
    const base = {
      indirizzoConsegna: "Via Validazione 4",
    ...
    expect(
      (
        await request(app)
          .post(`/bolle/${bolla}/converti-consegna`)
          .send({ ...base, dataPrevista: "2026-02-30" })
      ).status,
    ).toBe(400);
    expect(
      (
        await request(app)
          .post(`/bolle/${bolla}/converti-consegna`)
          .send({ ...base, mezzoAltro: true })
      ).status,
    ).toBe(400);

    const retainedBolla = await insertBolla(scope, {
      beneficiarioId: beneficiary,
      magazzinoId: warehouse,
      stato: "confermato",
      mezzoId: currentVehicle,
    ...
    await db
      ...
      .where(eq(bolleTable.id, retainedBolla));
    const retained = await request(app)
      .post(`/bolle/${retainedBolla}/converti-consegna`)
      .send(base);
    ...
    expect(
      (
        await db
          .select()
          .from(consegneTable)
          .where(eq(consegneTable.id, retained.body.consegnaId))
      )[0].mezzoId,
    ).toBe(currentVehicle);
    const retainedTurns = await db
      .select()
      .from(turniTable)
      .where(
        and(
          eq(turniTable.centroAscoltoId, centre),
          eq(turniTable.data, base.dataPrevista),
          eq(turniTable.fascia, "09-13"),
        ),
      );
    scope.turnoIds.push(...retainedTurns.map((turno) => turno.id));

    const withoutVehicle = await request(app)
      .post(`/bolle/${bolla}/converti-consegna`)
      .send({ ...base, mezzoId: null });
    ...
    expect(
      (
        await db
          .select()
          .from(consegneTable)
          .where(eq(consegneTable.id, withoutVehicle.body.consegnaId))
      )[0].mezzoId,
    ).toBeNull();
  });
  ...
  it("rifiuta il conflitto di mezzo sullo stesso giorno e fascia", async () => {
    ...
    const beneficiary = await createBeneficiario(scope, centre);
    const vehicle = await createMezzo(scope, { centroId: centre });
    await insertTurno(scope, {
      centroAscoltoId: otherCentre,
      data: "2026-09-06",
      fascia: "09-13",
      mezzoId: vehicle,
    });
    ...
    const response = await request(
      ...
      .post(`/bolle/${bolla}/converti-consegna`)
      .send({
        indirizzoConsegna: "Via Conflitto 6",
  ...
  it("rifiuta mezzo e volontario fuori dominio, fuori scope o oltre carico", async () => {
    ...
    await db
      ...
      .set({ attivo: false })
      .where(eq(volontariTable.id, inactiveVolunteer));
    await db
      .update(mezziTable)
      .set({ stato: "manutenzione" })
      .where(eq(mezziTable.id, otherVehicle));
    const app = makeScopedApp(bolleRouter, {
      id: operator,
      centroAscoltoId: centre,
    });
    const base = {
      indirizzoConsegna: "Via Scope 5",
      dataPrevista: "2026-09-05",
      fasciaOraria: "Mattina",
    };

    for (const extra of [
      { volontarioId: inactiveVolunteer },
      { volontarioId: otherVolunteer },
      { mezzoId: otherVehicle },
    ]) {
      const bolla = await insertBolla(scope, {
        beneficiarioId: beneficiary,
        magazzinoId: warehouse,
    ...
    await db
      .update(volontariTable)
      .set({ maxConsegneTurno: 1 })
      .where(eq(volontariTable.id, limited));
    const existing = await insertConsegna(scope, {
      beneficiarioId: beneficiary,
      magazzinoId: warehouse,
    ...
    await db
      .update(consegneTable)
      .set({ volontarioId: limited, fasciaOraria: "Mattina" })
      .where(eq(consegneTable.id, existing));
    const bolla = await insertBolla(scope, {
      beneficiarioId: beneficiary,
      magazzinoId: warehouse,
      stato: "confermato",
    ...
    expect(
      (
        await request(app)
          .post(`/bolle/${bolla}/converti-consegna`)
          .send({ ...base, volontarioId: limited })
      ).status,
    ).toBe(400);
  });
  ...
  it("impedisce di aggirare la conversione associando genericamente una bolla non ritirata", async () => {
    ...
    const response = await request(
      makeScopedApp(consegneRouter, { id: operator, centroAscoltoId: centre }),
    )
    ...
    expect(response.body.error).toContain(
      "conversione in consegna domiciliare",
    );
