...
import { db, pool, areeOperativeTable, ruoliTable, utentiTable } from "@workspace/db";
import { inArray } from "drizzle-orm";
import utentiRouter from "../src/routes/utenti";
import ruoliRouter from "../src/routes/ruoli";

const ids = { utenti: [] as number[], ruoli: [] as number[], areaOperativa: [] as number[] };
let areaA: number;
let areaB: number;
...
function appAs(user: { id: number; areaOperativaId: number | null; isSuperAdmin?: boolean; aree: string[]; ruoloId: number }): Express {
  const app = express();
  ...
  app.use((req, _res, next) => {
    req.user = {
      ...
      areaOperativaId: user.areaOperativaId,
      zonaUdsId: null,
...
beforeEach(async () => {
  ...
  const [a, b] = await db.insert(areeOperativeTable).values([{ nome: `Area A ${suffix}` }, { nome: `Area B ${suffix}` }]).returning({ id: areeOperativeTable.id });
  ...
  const [admin, other] = await db.insert(utentiTable).values([
    { username: `limited_${suffix}`, passwordHash: "x", nome: "Admin", cognome: "Limitato", ruoloId: limitedRole, areaOperativaId: areaA, matricola: `LIM-${suffix}` },
    { username: `other_${suffix}`, passwordHash: "x", nome: "Altro", cognome: "Utente", ruoloId: limitedRole, areaOperativaId: areaA, matricola: `OTH-${suffix}` },
...
describe("BUG 5.7 - amministratore territorialmente limitato", () => {
  const limitedApp = () => appAs({ id: limitedAdmin, areaOperativaId: areaA, ruoloId: limitedRole, aree: ["sociale"] });

  it("mantiene la propria Area A", async () => {
    const response = await request(limitedApp()).patch(`/utenti/${limitedAdmin}`).send({ areaOperativaId: areaA, nome: "Admin aggiornato" });
    expect(response.status).toBe(200);
    expect(response.body.areaOperativaId).toBe(areaA);
  });

  it("rifiuta Area B e accesso globale sul proprio profilo senza aggiornamenti parziali", async () => {
    const areaBResponse = await request(limitedApp()).patch(`/utenti/${limitedAdmin}`).send({ areaOperativaId: areaB, nome: "Non applicare" });
    expect(areaBResponse.status).toBe(403);
    const globalResponse = await request(limitedApp()).patch(`/utenti/${limitedAdmin}`).send({ areaOperativaId: null });
    ...
    expect(stored.areaOperativaId).toBe(areaA);
    expect(stored.nome).toBe("Admin");
  });

  it("rifiuta Area B su un altro utente e un ruolo funzionale più ampio", async () => {
    expect((await request(limitedApp()).patch(`/utenti/${otherUser}`).send({ areaOperativaId: areaB })).status).toBe(403);
  ...
  it("consente al Super Admin di assegnare Area B", async () => {
    const superApp = appAs({ id: 999999, areaOperativaId: null, ruoloId: widerRole, aree: ["sociale", "amministrazione"], isSuperAdmin: true });
    const response = await request(superApp).patch(`/utenti/${otherUser}`).send({ areaOperativaId: areaB });
    expect(response.status).toBe(200);
    expect(response.body.areaOperativaId).toBe(areaB);
  });

  it("rifiuta un'area inesistente", async () => {
    const superApp = appAs({ id: 999999, areaOperativaId: null, ruoloId: widerRole, aree: ["sociale", "amministrazione"], isSuperAdmin: true });
    expect((await request(superApp).patch(`/utenti/${otherUser}`).send({ areaOperativaId: 2_000_000_000 })).status).toBe(400);
  });
});
