 ...
 *
 * A global admin (areaOperativaId = null) may narrow any report to a single area operativa via
 * ?areaOperativaId. A scoped caller stays HARD-bound to its own area operativa: the ?areaOperativaId
 * param is ANDed on top of the existing own-area operativa-or-null scope, so passing
 * another area operativa's id can only ever shrink the result to zero — never leak.
 */
