...
async function createArea(nome: string, attivo = true): Promise<number> {
  const [row] = await db.insert(areeOperativeTable).values({ nome: `${nome}-${suffix}`, attivo }).returning({ id: areeOperativeTable.id });
  areaIds.push(row.id);
  return row.id;
}

async function createCenter(areaOperativaId: number, nome: string, attivo = true): Promise<number> {
  const [row] = await db.insert(centriAscoltoTable).values({ nome: `${nome}-${suffix}`, areaOperativaId, attivo }).returning({ id: centriAscoltoTable.id });
  centerIds.push(row.id);
  return row.id;
}

async function createZone(areaOperativaId: number, nome: string): Promise<number> {
  const [row] = await db.insert(zoneUdsTable).values({ nome: `${nome}-${suffix}`, areaOperativaId }).returning({ id: zoneUdsTable.id });
  zoneIds.push(row.id);
  return row.id;
}

async function createWarehouse(areaOperativaId: number, nome: string): Promise<number> {
...
afterAll(async () => {
  ...
  if (areaIds.length) await db.delete(areeOperativeTable).where(inArray(areeOperativeTable.id, areaIds));
});

describe("hardening Beneficiari: RBAC, DTO e scope", () => {
  it("nega lettura e mutazione senza permessi granulari", async () => {
    const app = makeApp({ areaOperativaId: areaA, centroAscoltoId: null, zonaUdsId: null, aree: ["sociale"], permessi: [] });
    expect((await request(app).get("/beneficiari")).status).toBe(403);
    expect((await request(app).post("/beneficiari").send({ nome: "No", cognome: "Permesso", sesso: "M", areaOperativaId: areaA })).status).toBe(403);
  });
  ...
  it("rifiuta mass assignment tecnico/economico in create e patch", async () => {
    const create = await request(socialAreaA()).post("/beneficiari").send({
      nome: "Mass", cognome: "Assignment", sesso: "M", areaOperativaId: areaA, creditoSolidaleSaldo: 999,
    });
  ...
  it("richiede Area e rifiuta relazioni territoriali incoerenti o inattive", async () => {
    ...
    expect((await request(makeApp({ areaOperativaId: null, centroAscoltoId: null, zonaUdsId: null, aree: ["sociale"], permessi: SOCIAL_PERMISSIONS })).post("/beneficiari").send(base)).status).toBe(400);
    ...
    expect((await request(makeApp({ areaOperativaId: null, centroAscoltoId: null, zonaUdsId: null, aree: ["sociale"], permessi: SOCIAL_PERMISSIONS })).post("/beneficiari").send({ ...base, areaOperativaId: inactiveArea })).status).toBe(400);
  });
  ...
  it("consente all'Admin globale di creare un Beneficiario Sociale nell'Area scelta", async () => {
    ...
    expect(created.body.areaOperativaId).toBe(areaA);
  ...
  it("offre directory paginata minimale e non supera l'Area", async () => {
    ...
    await insertBeneficiary({ areaOperativaId: areaB });
    ...
    expect((await request(socialAreaA()).get("/beneficiari").query({ areaOperativaId: "abc" })).status).toBe(400);
  });
});
...
describe("hardening Beneficiari: privacy, locking e storico", () => {
  ...
  it("impedisce a UDS di scrivere campi sociali sensibili", async () => {
    const app = makeApp({ areaOperativaId: areaA, centroAscoltoId: null, zonaUdsId: zoneA, aree: ["uds"], permessi: ["beneficiari.view", "beneficiari.manage"] });
  ...
  it("autorizza e audita export senza copiare dati sensibili", async () => {
    const denied = makeApp({ areaOperativaId: areaA, centroAscoltoId: null, zonaUdsId: null, aree: ["sociale"], permessi: ["beneficiari.view"] });
...
describe("hardening Beneficiari: nucleo e bulk", () => {
  it("non crea membri orfani e distingue 404 da 403", async () => {
    ...
    const foreign = await insertBeneficiary({ areaOperativaId: areaB });
