...
describe("Volontari 2.0 — regole di dominio", () => {
  ...
  it("rende operativo un permanente approvato, abilitato e assicurato", () => {
    expect(evaluateOperationalState({
      ...approved,
      coperture: [{ dataInizio: "2027-01-01", dataFine: "2027-12-31", annullata: false }],
    })).toMatchObject({ operativo: true, motivoNonOperativo: null, statoAssicurazione: "VALIDA" });
  });

  it("rende non operativo senza job quando l'assicurazione è scaduta", () => {
    expect(evaluateOperationalState({
      ...approved,
      coperture: [{ dataInizio: "2026-01-01", dataFine: "2027-03-14", annullata: false }],
    })).toMatchObject({ operativo: false, motivoNonOperativo: "ASSICURAZIONE_SCADUTA", statoAssicurazione: "SCADUTA" });
  });

  it("non rende operativo un temporaneo fuori dalla giornata registrata", () => {
    const coverage = [{ dataInizio: "2027-01-01", dataFine: "2027-12-31", annullata: false }];
    expect(evaluateOperationalState({ ...approved, tipoVolontario: "TEMPORANEO", coperture: coverage }))
      .toMatchObject({ operativo: false, motivoNonOperativo: "GIORNATA_TEMPORANEA_MANCANTE" });
    expect(evaluateOperationalState({ ...approved, tipoVolontario: "TEMPORANEO", giornataValida: true, coperture: coverage }))
      .toMatchObject({ operativo: true, giornataTemporaneaValida: true });
  });

  it("la riattivazione non supera approvazione o assicurazione scaduta", () => {
    expect(evaluateOperationalState({ ...approved, approvazione: "in_attesa", coperture: [] }))
      .toMatchObject({ operativo: false, motivoNonOperativo: "IN_ATTESA_APPROVAZIONE" });
    expect(evaluateOperationalState({ ...approved, coperture: [] }))
      .toMatchObject({ operativo: false, motivoNonOperativo: "ASSICURAZIONE_MANCANTE" });
  });
  ...
  it("una copertura non annulla una sospensione manuale", () => {
    expect(evaluateOperationalState({
      ...
      coperture: [{ dataInizio: "2027-01-01", dataFine: "2027-12-31", annullata: false }],
    })).toMatchObject({ operativo: false, motivoNonOperativo: "SOSPENSIONE_MANUALE", sospesoManualmente: true });
  });

  it("normalizza chiavi anagrafiche e ruoli senza duplicati di case o spazi", () => {
    expect(normalizeCodiceFiscale(" rss mra 80a01 h501u ")).toBe("RSSMRA80A01H501U");
